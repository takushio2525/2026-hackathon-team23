...
def graph_mop4_sync_error_slide():
    ...
    同期誤差は「同一拍における 5 台の発火時刻レンジ」でノード別に分解できないため、
    x 軸は M45F 方式で実測した 3 回 (対策前 → 対策後 → 最終構成) の計測回別とする
    (MOP_REPORT_20260711.md の 3 回実測比較表と同じ並び)。体裁は MOP5 スライド版と
    同一: 平均値 (青棒)・中央値 (白抜き◇)・最大値 (赤マーク) を 16:9 横長・
    大きめフォントで描き、解説は口頭で行う前提のためサブタイトル・判定ボックス・
    注記・出典行は載せない。
    """
    threshold_ms = 20
    run_specs = [
        ('20260710_221532', '対策前\n(7/10)'),
        ('20260710_231356', '対策後\n(7/10)'),
        ('20260711_154006', '最終構成\n(7/11)'),
    ]
    runs = []
    for stem, label in run_specs:
        path = RESULTS_DIR / 'mop4' / f'{stem}.csv'
        if not path.exists():
            print(f'  mop4/{stem}.csv なし、スライド版はスキップ')
            return
        ...
        for r in read_csv(path):
            ...
            groups[(int(r['beatNo']), int(r['playAtMasterMs']))].append(
                float(r['localMasterMs']))
        errors = np.array([max(v) - min(v) for v in groups.values() if len(v) >= 2])
        runs.append((label, errors))

    labels = [label for label, _ in runs]
    means = [float(np.mean(e)) for _, e in runs]
    p50s = [float(np.percentile(e, 50)) for _, e in runs]
    maxs = [float(np.max(e)) for _, e in runs]
    ...
    for xi, v in enumerate(means):
        ax.text(xi, v + 1.4, f'{v:.1f}', ha='center', va='bottom',
    ...
    # 中央値 7〜8 ms は青棒 (平均 9.8〜10.8 ms) の内側に来るため、白抜き◇＋濃い縁で
    ...
    ax.set_ylabel('楽器間の発音時刻差 (ms)', fontsize=17)
    ax.set_ylim(0, 104)
    ax.grid(axis='y', alpha=0.3, zorder=0)

    ax.set_title('楽器間同期誤差（計測回別）', fontsize=21,
