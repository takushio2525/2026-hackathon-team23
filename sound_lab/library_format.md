...
```jsonc
{
  ...
  "instrument_profile": "auto",  // 解析時の楽器プロファイル（auto / trumpet / flute / organ / drum など）
  ...
  "attack_sample": { ... },       // 任意。トランペット / フルート / オルガン / ドラム指定時の原音アタック波形（下記）
  "sustain_sample": { ... },      // 任意。トランペット / フルート / オルガン指定時の原音定常ループ（下記）
  "tone_sample": { ... },         // 任意。フルート指定時の原音本体サンプル（下記）
...
トランペット / フルート / オルガン / ドラム指定で解析したときだけ入る。サイン波の倍音加算では再現しにくい、
タンギング直後の息圧、唇のバズ感、キークリック、打面のクリック、胴鳴りの出だし、高域の立ち上がりを補うため、
トリム後の原音先頭を短く正規化して保存する。
...
トランペットでは通常 0.18 秒、フルートでは通常 0.16 秒、オルガンでは通常 0.08 秒、ドラム / 打楽器では通常 0.45 秒を保存する。
ブラウザスタジオでは `fx.attack_sample_mix` と「原音アタックを重ねる」スライダーで量を調整できる。
Processing 現行版は未対応なので、この欄があっても無視して従来どおり鳴る。

## `sustain_sample` — 原音定常ループ（任意）

`トランペット` / `フルート` / `オルガン` プロファイルで解析したときだけ入る。サイン波の倍音加算では再現しにくい、
伸びている音の中にある唇の細かいバズ、管鳴り、息の芯、オルガンの定常倍音を補うため、
定常部から基音周期の整数倍に近い短い波形を切り出して保存する。
...
## `tone_sample` — 持続楽器の原音本体サンプル（任意）

`フルート` プロファイルで解析したときだけ入る。フルートは倍音加算だけで再合成すると原音の息の質感や揺れが崩れやすいため、
トリム後の原音本体を最大 4 秒だけ 22.05 kHz で保存し、ブラウザスタジオではこれを主音として鳴らす。
倍音・ノイズ合成は表示や軽い補助として残す。
...
"tone_sample": {
  ...
  "loop_start_sec": 1.42,
  "loop_end_sec": 2.14,
  "loop_start_sample": 31311,
  "loop_end_sample": 47187,
  "source_peak": 0.51,
  "values": [0.0, 0.012, ...]
}
```

ブラウザスタジオでは `fx.tone_sample_mix` と「原音サンプルを主音にする」スライダーで量を調整できる。
`loop_start_sample` / `loop_end_sample` がある場合は、ブラウザスタジオは秒数よりサンプル番号を優先してループ境界を決める。
Processing 現行版は未対応なので、この欄があっても無視して従来どおり鳴る。
...
`tone_sample_mix` は、`tone_sample` を主音として鳴らす量。フルート指定時のブラウザスタジオ初期値は 1.0。
