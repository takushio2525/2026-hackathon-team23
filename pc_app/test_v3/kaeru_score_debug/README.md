...
このフォルダは、作成済みの「かえるのうた」楽譜と解析済み音色 JSON を
`pc_app/test_v3` 側で確認・移植するための試聴用 Processing スケッチです。
本体の `orchestra_resynth/` に統合し終わったあと、不要になったら削除して構いません。
...
ドラムはチューバと、8拍遅れて始まるホルンが終止するまでの40拍を演奏します。
各拍にクローズドハイハットを置き、奇数拍側をキック、偶数拍側をスネアで
支える単純なリズム伴奏です。8拍ごとの区切りではスネアとハイハットをわずかに強め、
主旋律が3声そろう16〜31拍目も、全体として主旋律を邪魔しない音量に抑えます。
終止前の36〜39拍目はハイハットを弱め、特別な締めを入れずに通常のリズムのまま
40拍目でホルンとチューバと同時に終わります。
...
| キック | `kick.tweaked.instrument.json` |
| スネア | `snare.tweaked.instrument.json` |
| ハイハット | `Hi-hat.tweaked.instrument.json` |
| クラッシュ | `crash.tweaked.instrument.json` |
...
ドラムも解析 JSON を読み込み、倍音、エンベロープ、ノイズ量を使って
キック、スネア、ハイハット、クラッシュを控えめなバランスで鳴らします。

JSON に含まれる `waveform`, `noise`, `modulation`, `fx` は元データとして保持しますが、
この試聴用スケッチでは `noise.level` 以外はまだ合成に使用していません。

## 実行方法

1. Processing 4 で `kaeru_score_debug.pde` を開く。
2. Minim ライブラリが未導入の場合は、Contribution Manager から `Minim` を追加する。
3. Run 後、5パートが1回自動再生される。再度聞く場合は `P` キーで5パートの再生、
   `1`〜`5` キーで各パート単独再生を行う。
...
差し替えます。チューバとドラムは `startBeatNo = 0` で40拍の配列を使用し、
8拍遅れて始まるホルンと同じ時点で終わります。
