...
// 日本語表示用フォント。Processing のデフォルト sans フォントは CJK を持たないため
// setup() で OS インストール済みフォントを動的に読み込んで全体に適用する。
PFont uiFont;
...
// 内蔵メロディ (firmware/test_v1/node_02/src/score_data.cpp の冒頭と同じ「ドレミファミレド」)
...
void setup() {
    ...
    // 日本語表示用フォントをロード。失敗時はデフォルトフォントのままにする
    // (英数字は正しく出るので致命傷ではない)。
    uiFont = loadJapaneseFont(14);
...
// OS にインストールされている日本語対応フォントを優先順位付きで探してロードする。
// PFont.list() で利用可能なフォント名一覧を取得し、先頭から一致するものを使う。
PFont loadJapaneseFont(float sizePx) {
    String[] candidates = {
        "Hiragino Sans",                 // macOS (Big Sur 以降)
        "Hiragino Kaku Gothic ProN",     // macOS (旧版)
        "HiraginoSans-W3",               // PostScript 名
        "Yu Gothic",                     // Windows
        "Yu Gothic Medium",
        "Meiryo",                        // Windows (旧)
        "Noto Sans CJK JP",              // Linux / フォント配布
        "Noto Sans JP",
        "Arial Unicode MS"
    };
    ...
    for (String c : candidates) {
        for (String a : avail) {
            if (a.equalsIgnoreCase(c)) {
                println("UI font: " + c);
                // 第3引数 smooth=true でアンチエイリアス有効
                return createFont(c, sizePx, true);
            }
        }
    }
