...
\title{ハッカソン}
\subtitle{Processing 音源サブシステム\\基本設計・詳細設計 改良版}
\team{チーム23}
\author{
  25G1021 : 梅澤 颯太
}
...
\chapter{本書の位置づけ}\setcounter{page}{1}

\section{目的}
本書は，チーム23「Arduino オーケストラ」における梅澤担当範囲，
すなわち Processing 側の NOTE 受信，音色合成，発音管理，最小 UI，ログ出力を対象とした
基本設計・詳細設計である．

初版の設計書では，金管音色の方針は示せていたが，ルーブリックで要求される
機能一覧，操作インターフェース，状態遷移，ハードウェア接続，NOTE 受信処理，
ボイス管理，例外処理の粒度に不足があった．
本改良版では，既に作成した
\path{work/umezawa/hck/week3/orchestra_processing_final/}
の実装内容を正として，実装と矛盾しない設計書へ更新する．
...
\caption{梅澤担当範囲と本書での対応}\label{tab:wbs}\\
\toprule
WBS & タスク & 本書での設計箇所 \\
\midrule
\endfirsthead
...
\section{評価基準への対応方針}
本書は，システム設計書ルーブリックの5項目に対応するため，以下を必ず記載する．

\begin{itemize}
  \item 基本設計：機能一覧とシステム構成図
  \item 基本設計：操作インターフェースと状態遷移
  \item 詳細設計：ハードウェア設計
  \item 詳細設計：ソフトウェア設計
  \item 詳細設計：処理フローの設計
\end{itemize}
...
\section{機能一覧}
Processing 音源サブシステムを F6 として定義し，塩澤担当の F4「発音情報配信」
の受け口以降を担当する．表\ref{tab:fbs}に機能一覧を示す．

\begin{longtable}{p{0.11\linewidth}p{0.20\linewidth}p{0.49\linewidth}p{0.10\linewidth}}
...
\endhead
F6.1 & Serial 接続管理 &
Processing 起動時に Serial ポート一覧を表示し，画面上のポートをクリックして接続する．切断時は再接続待ちへ戻す． & 124 \\
F6.2 & NOTE フレーム同期・復号 &
USB Serial のバイト列から \texttt{magic=0x4F52} を探し，20 byte 固定長 NOTE に復号する． & 124 \\
F6.3 & パート設定管理 &
期待 \texttt{partId} を \texttt{0x02--0x05} から選び，誤接続時は発音せず警告とログを残す．検証用に ALL モードも持つ． & 125 \\
F6.4 & ボイス管理 &
NoteOn，NoteOff，\texttt{durationMs} 経過による自動消音，同時発音数上限，古い Voice の解放を管理する． & 125 \\
F6.5 & 音色生成 &
\texttt{0x02--0x04} は金管音色，\texttt{0x05} はリズム音色として合成し，Minim の AudioOutput へ出力する． & 117/126 \\
F6.6 & UI 表示・操作 &
接続状態，期待 partId，受信数，破棄数，発音中 Voice 数，波形，警告を表示し，テスト発音やミュートを操作できるようにする． & 254 \\
F6.7 & ログ・検証支援 &
受信時刻，NOTE 内容，破棄理由，発音開始を CSV に記録し，結合試験で欠落や誤接続を確認できるようにする． & 124/125 \\
\bottomrule
\end{longtable}
...
\section{ハードウェア設計}
Processing 側では回路を追加しないが，ルーブリックのハードウェア設計項目に対応するため，
接続される構成要素と故障時動作を明確化する．
...
P4 & スピーカまたは内蔵スピーカ & 4 & Processing の AudioOutput を再生する． \\
P5 & Processing 4 + Minim & 4環境 & NOTE を受けて音を合成する実行環境． \\
\bottomrule
\end{longtable}
...
\section{ソフトウェア設計}
\subsection{ファイル構成}
実装は \path{work/umezawa/hck/week3/orchestra_processing_final/} に配置する．

\begin{lstlisting}[style=textdiagram, caption={Processing 実装ファイル構成}, label={lst:filetree}]
orchestra_processing_final/
|-- orchestra_processing_final.pde  // setup(), draw(), serialEvent(), key/mouse input
|-- Config.pde                      // serial, protocol, partId, voice constants
|-- Protocol.pde                    // NotePacket, little-endian decoder, test frame
|-- SerialFrameReader.pde           // byte buffer and magic sync
|-- PartManager.pde                 // part voice allocation and voice limits
|-- Voice.pde                       // Voice, BrassVoice, RhythmVoice
|-- UiView.pde                      // status screen, clickable port list, waveform
|-- Logger.pde                      // CSV logging
|-- README.md                       // operation note
`-- sketch.properties               // Processing Java mode
\end{lstlisting}
...
\midrule
\endhead
PT-1 & ビルド確認 & \texttt{processing-java --build} を実行する． & エラーなく完了する． \\
PT-2 & NOTE 復号単体 & \texttt{g} キーで疑似 NOTE を注入する． & 受信数が増え，テスト音が鳴る． \\
PT-3 & ポート選択 UI & Serial ポート一覧をクリックする． & 接続状態が Ready になり，ポート名が表示される． \\
PT-4 & partId 誤接続 & 期待 partId と違う NOTE を入力する． & 発音せず wrong part と表示・ログ出力する． \\
PT-5 & NoteOn 自動停止 & \texttt{durationMs=500} の NOTE を入力する． & 約500 ms後に release され，鳴りっぱなしにならない． \\
PT-6 & 同時発音上限 & 同一 partId で5音以上を連続入力する． & 古い Voice が release され，上限4を維持する． \\
PT-7 & リズム音色 & partId 0x05 で noteNumber 36，38，42 を入力する． & 短いリズム音として鳴る． \\
PT-8 & Serial 再接続 & \texttt{d} または \texttt{r} を押して再接続する． & キューとバッファがクリアされ，再接続できる． \\
PT-9 & 結合演奏 & node\_02--05 と Mac 4台を接続し演奏する． & 欠落なく鳴り，誤接続警告が出ない． \\
\bottomrule
\end{longtable}

\section{ルーブリック対応}
\begin{longtable}{p{0.32\linewidth}p{0.50\linewidth}p{0.10\linewidth}}
...
\endhead
基本設計：機能一覧とシステム構成図 &
第2章で FBS，Mac 4台構成図，partId 対応表を示した． & A \\
基本設計：操作インターフェースと状態遷移 &
第3章でクリック式ポート選択，キー操作，状態遷移図，状態定義を示した． & A \\
詳細設計：ハードウェア設計 &
第4章で Arduino，Mac，USB，スピーカ，通信方式，故障時動作を示した． & A \\
詳細設計：ソフトウェア設計 &
第4章で実装ファイル構成，NOTE パケット，クラス設計，音色パラメータを示した． & A \\
詳細設計：処理フローの設計 &
第5章で起動，Serial 受信キュー，パケット検査，ボイス管理，例外処理を示した． & A \\
\bottomrule
\end{longtable}

\chapter{生成AIの利用について}
本設計書の改良にあたり，生成AIを利用した．利用範囲は，既存設計書，レビュー資料，
塩澤設計，実装済み Processing プログラムの読み取り補助，章立ての整理，
ルーブリック項目との対応確認，文章案の作成である．

ただし，採用する前提条件，NOTE パケット仕様，Mac 4台構成，partId 対応，音色パラメータ，
テスト項目については，リポジトリ内資料および実装済みプログラムを確認したうえで判断した．
生成AIの出力はそのまま採用せず，塩澤・齋藤の仕様および梅澤担当範囲と矛盾しないよう修正した．
