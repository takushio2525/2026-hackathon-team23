...
Processing の `instrument_player` は、音響結果に関係する全項目を読み込む（`note` と
`balance_export_note` は説明用メタデータなので発音には使わない）。
...
Processing の `instrument_player` は `sample_rate` と `root_midi_note` に従ってリサンプリングし、
`fx.attack_sample_mix` の量で発音先頭へ重ねる。
...
Processing の `instrument_player` は指定ループ区間を繰り返し、`fx.sustain_sample_mix` の量で重ねる。
...
元の1打のピッチで鳴る。
Processing の `instrument_player` も同じ選択規則で再生する。
...
Processing の `instrument_player` も音響項目を再現する。スタジオ側はこの欄を読み戻して
...
`fx.note` と `fx.balance_export_note` は表示・追跡用で音には影響しない。
`fx.master_volume` は単一楽器スタジオのマスター音量で、Processing でもFX後段へ適用する。
`fx.studio_state` はUIで設定した全項目の監査・再現確認用スナップショット。`brightness`、倍音ゲイン、
`noiseLevel` などは `harmonics[]` / `noise.level` へ既に畳み込み済みなので、再生側は `studio_state` を重ねて適用しない。
`fx.glide_ms` は Processing では直前に発音したノートから次のノートへのポルタメントとして適用する。
ブラウザ版は鳴っている Voice の音程を操作した場合の追従時間として使うため、同時発音時の開始音程は実装間で異なる。
