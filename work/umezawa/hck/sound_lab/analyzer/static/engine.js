...
(function () {
  ...
  class Voice {
    ...
    applyFrequencies(immediate) {
      ...
      for (const h of this.harm) {
        ...
        const inharmB = P.testMultiCompat ? Math.max(0, S.instrument.inharmonicity_b || 0) : 0;
        const inharmonicFactor = Math.sqrt(1 + inharmB * pure * pure);
        const f = clamp(this.baseF * trFactor * ratio * inharmonicFactor, 1, S.ctx.sampleRate * 0.49);
    ...
    scheduleAmp(t0) {
      ...
      if (useRecorded && I.sustaining) {
        ...
        if (P.testMultiCompat) {
          // pc_app/common/SynthVoice.pde の simpleADSR と同じ線形 A/D。
          g.setValueAtTime(0, t0);
          g.linearRampToValueAtTime(peak, t0 + A);
          g.linearRampToValueAtTime((I.sustaining ? Sl : 0.02) * peak, t0 + A + D);
          this.naturalEnd = I.sustaining ? Infinity : t0 + A + D + 0.05;
          this.lastBodyLvl = (I.sustaining ? Sl : 0.02) * peak;
        } else if (P.attackCurve === "exp") { g.setValueAtTime(0.0008, t0); g.exponentialRampToValueAtTime(Math.max(peak, 0.001), t0 + A); }
        else { g.setValueAtTime(0, t0); g.linearRampToValueAtTime(peak, t0 + A); }
        if (P.testMultiCompat) {
          // 上の分岐で設定済み。
        } else if (I.sustaining) {
    ...
    noteOff(when) {
      ...
      if (P.testMultiCompat) {
        // Processing 側は releaseStartLevel * (1-u)^2。
        const elapsed = Math.max(0, t - this.t0);
        const A = Math.max(0.001, (P.attackMs || 5) / 1000);
        const D = Math.max(0.003, (P.decayMs || 60) / 1000);
        const sustain = clamp(P.sustainLvl == null ? 0.7 : P.sustainLvl, 0, 1) * this.vel;
        const start = elapsed < A
          ? this.vel * elapsed / A
          : elapsed < A + D
            ? this.vel + (sustain - this.vel) * ((elapsed - A) / D)
            : sustain;
        const curve = new Float32Array(65);
        for (let i = 0; i < curve.length; i++) { const u = i / (curve.length - 1); curve[i] = start * (1 - u) * (1 - u); }
        curve[curve.length - 1] = 0;
        this.amp.gain.setValueCurveAtTime(curve, t, relSec);
        this.releaseEnd = t + relSec + 0.02;
      } else {
  ...
  SL.LiveSynth = class LiveSynth {
    ...
    _initNoiseBuffer() {
      ...
      const noise = this.instrument && this.instrument.noise || {};
      const edges = noise.bands_hz || [], levels = noise.band_levels || [];
      if (levels.length && edges.length === levels.length + 1) {
        // Processing版のFFT帯域整形に近づけるため、同じ白色ノイズを複数の
        // 1-pole LP差分へ通し、各帯域をband_levelsで重み付けして足し直す。
        for (let band = 0; band < levels.length; band++) {
          const lo = clamp(+edges[band] || 0, 0, sr*0.49);
          const hi = clamp(+edges[band+1] || sr*0.49, Math.max(lo+1, 1), sr*0.49);
          const aLo = lo > 0 ? 1-Math.exp(-2*Math.PI*lo/sr) : 0;
          const aHi = 1-Math.exp(-2*Math.PI*hi/sr);
          let stateLo=0, stateHi=0;
          const weight = Math.max(0, +levels[band] || 0);
          for (let i=0;i<len;i++) {
            const x=white[i];
            stateHi += aHi*(x-stateHi);
            if (aLo>0) stateLo += aLo*(x-stateLo);
            d[i] += (stateHi-stateLo)*weight;
          }
        }
        let peak=1e-9;
    ...
    setDrone(on, midi) {
      ...
      if (this.drone) { this.releaseAllHeld(); this._droneVoice = this.noteOn(this.droneMidi, this.params.testMultiCompat ? 0.92 : 0.85); }
    ...
    _retriggerDrone() {
      ...
      const v = new Voice(this, this.droneMidi, this.params.testMultiCompat ? 0.92 : 0.85);
    ...
    exportInstrument() {
      ...
      // ブラウザスタジオで足したエフェクト。instrument_player は音響項目をすべて使用する。
      out.fx = {
        note: "sound_lab スタジオで設定。instrument_player は音響項目に対応。note と balance_export_note はメタデータ。",
        master_volume: P.masterVol,
        ...
        // UIで設定した生の値も全項目保存する。brightness等は上でharmonics/noiseへ
        // 畳み込み済みなので、再生側はstudio_stateではなく本体値とfxを使用する。
        studio_state: {
          version: 1,
          masterVol: P.masterVol, transposeSemis: P.transposeSemis, glideMs: P.glideMs, humanizeCents: P.humanizeCents,
          drumSampleMix: P.drumSampleMix, drumPitchFollow: P.drumPitchFollow,
          attackSampleMix: P.attackSampleMix, trumpetWaveMix: P.trumpetWaveMix, sustainSampleMix: P.sustainSampleMix,
          brassLayerMix: P.brassLayerMix, brassLayerDetuneCents: P.brassLayerDetuneCents, trumpetResonance: P.trumpetResonance,
          fineCents: P.fineCents, vibDepthCents: P.vibDepthCents, vibRateHz: P.vibRateHz, vibOnsetSec: P.vibOnsetSec, vibShape: P.vibShape,
          brightness: P.brightness, harmRolloff: P.harmRolloff, oddEvenBal: P.oddEvenBal, inharmMul: P.inharmMul,
          harmLimit: P.harmLimit, harmFollowEnv: P.harmFollowEnv, harmGains: Object.assign({}, P.harmGains || {}),
          tremDepth: P.tremDepth, tremRateHz: P.tremRateHz, tremShape: P.tremShape,
          noiseLevel: P.noiseLevel, breathAmount: P.breathAmount, noiseHpHz: P.noiseHpHz, noiseLpHz: P.noiseLpHz,
          attackNoise: P.attackNoise, noiseMode: P.noiseMode,
          envMode: P.envMode, attackMs: P.attackMs, decayMs: P.decayMs, sustainLvl: P.sustainLvl,
          releaseMs: P.releaseMs, attackCurve: P.attackCurve, decayStretch: P.decayStretch,
          reverbMix: P.reverbMix, reverbSizeSec: P.reverbSizeSec, reverbDamping: P.reverbDamping,
          reverbPreMs: P.reverbPreMs, reverbWidth: P.reverbWidth,
          driveAmount: P.driveAmount, driveToneHz: P.driveToneHz,
          chorusMix: P.chorusMix, chorusRateHz: P.chorusRateHz, chorusDepth: P.chorusDepth, chorusWidth: P.chorusWidth,
          filterMode: P.filterMode, filterCutoffHz: P.filterCutoffHz, filterQ: P.filterQ,
          filterLfoRateHz: P.filterLfoRateHz, filterLfoDepth: P.filterLfoDepth,
          eqLowGain: P.eqLowGain, eqMidFreq: P.eqMidFreq, eqMidGain: P.eqMidGain,
          eqMidQ: P.eqMidQ, eqPresGain: P.eqPresGain, eqHighGain: P.eqHighGain,
        },
      };
  ...
  // test_multi の基本項目に加え、音色改善版が使う倍音 env / noise / body_eq も残す。
  SL.toTestMultiInstrument = function (instrument, target) {
    const out = JSON.parse(JSON.stringify(instrument));
    const kind = target === "horn" ? "horn" : "trumpet";
    out.instrument_profile = kind;
    out.instrument_profile_label = kind === "horn" ? "ホルン" : "トランペット";
    out.harmonics = (out.harmonics || []).map((h) => {
      const clean = { n: h.n, ratio: h.ratio, amp: h.amp };
      if (h.amp_db != null) clean.amp_db = h.amp_db;
      if (h.phase != null) clean.phase = h.phase;
      clean.env = Array.isArray(h.env) && h.env.length >= 2 ? h.env.slice() : [1, 1];
      return clean;
    });
    if (!out.noise) {
      out.noise = {
        level: 0, rate_hz: 200, envelope: [1, 1],
        bands_hz: [0, 125, 250, 500, 1000, 2000, 4000, 8000, 16000, 22050],
        band_levels: [1, 1, 1, 1, 1, 1, 1, 1, 1],
      };
    }
    const sourceFx = out.fx || {};
    const sourceEq = sourceFx.body_eq || {};
    const dynamicHarmonics = sourceFx.harm_follow_env !== false;
    out.fx = {
      note: "test_multi_sound_improved対応項目のみ。body_eqとtrumpet_resonanceはProcessingでも再生されます。",
      harm_follow_env: dynamicHarmonics,
      trumpet_resonance: +sourceFx.trumpet_resonance || 0,
      body_eq: {
        low_gain: +sourceEq.low_gain || 0,
        mid_freq: +sourceEq.mid_freq || 900,
        mid_gain: +sourceEq.mid_gain || 0,
        mid_q: +sourceEq.mid_q || 1,
        presence_gain: +sourceEq.presence_gain || 0,
        high_gain: +sourceEq.high_gain || 0,
      },
    };
    delete out.attack_sample;
    delete out.sustain_sample;
    delete out.drum_sample;
    delete out.waveform;
    delete out.modulation;
    out.test_multi_compat = {
      version: 2,
      synthesis: "dynamic_harmonics_linear_adsr_body_fx",
      dynamic_harmonics: dynamicHarmonics,
      noise: true,
      body_fx: true,
      target_path: "work/umezawa/hck/test_multi_sound_improved/orchestra_test_sound_improved/data/",
    };
    return out;
  };
})();
