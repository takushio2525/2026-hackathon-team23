...
Arduino などから送られてくる 20 byte の NOTE フレームを Processing 側で受け取り、Minim を使って音を鳴らします。4つの金管パートと1つのリズムパートを扱う構成です。
...
| キー | 動作 |
| --- | --- |
| `1` | 受け付けるパートを `Tuba`、`partId = 0x02` にする |
| `2` | 受け付けるパートを `Trombone`、`partId = 0x03` にする |
| `3` | 受け付けるパートを `Horn`、`partId = 0x04` にする |
| `4` | 受け付けるパートを `Rhythm`、`partId = 0x05` にする |
| `5` | 受け付けるパートを `Trumpet`、`partId = 0x06` にする |
| `a` | 全パートを受け付けるモードに切り替える |
| `m` | ミュートのオン/オフを切り替える |
| `t` | 現在選択中のパートでテスト音を鳴らす |
| `g` | 現在選択中のパートで疑似 NOTE フレームを作り、受信処理に流す |
| `r` | 演奏画面ではポート再選択、ポート選択画面では一覧を更新する |

キー操作は演奏画面で使えます。通常の確認では、まず `1` から `5` で確認したいパートを選び、`t` で音が鳴るかを見ると分かりやすいです。複数パートをまとめて受信したいときは `a` を押します。
...
`keyPressed()` には演奏画面で使うキー操作がまとまっています。`1` から `5` のパート切り替え、`a` の全パート受信、`m` のミュート、`t` のテスト音、`g` の疑似 NOTE などはここで処理されます。
...
- それ以外の場合: `BrassProfile` 付きの `BrassVoice`

金管4パートは同じ `BrassVoice` クラスを使いますが、`partId` ごとに `BrassProfile` を切り替えます。`BrassProfile` では波形、立ち上がり、減衰、伸び、消え方、音量、音域補正を変えています。

`playTestNote()` は `t` キーで使われます。各楽器で聞き分けやすい基準音を選んで鳴らします。
...
5つのパートIDは `P02_ConstantsAndParts.pde` にあります。
...
このプログラムでは、`PART_BRASS_1` を Tuba、`PART_BRASS_2` を Trombone、`PART_BRASS_3` を Horn、`PART_BRASS_4` を Trumpet として扱います。
...
このため、4つの金管パートは同じ `BrassVoice` を使いながら、`Tuba`、`Trombone`、`Horn`、`Trumpet` ごとに違う設定で鳴ります。金管音の波形とADSR設定は `P05_SoundPartManager.pde` の各 `BrassProfile` で作っています。
...
| 金管の音色を変えたい | `P05_SoundPartManager.pde` の各 `BrassProfile`、`P06_SoundVoices.pde` の `BrassVoice` |
...
`1` から `5` キーで期待するパートを切り替えるか、`a` キーで全パートを受け付けるモードにしてください。
