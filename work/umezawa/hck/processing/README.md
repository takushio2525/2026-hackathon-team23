...
4. 実行ボタンを押す。
5. 画面左側の `Serial` 一覧から Arduino のポートをクリックする。
6. 接続できると、画面右側の `state` が `Ready` になります。
7. Arduino から NOTE フレームが届くと、`state` が `Playing` になり音が鳴ります。
...
通常の確認では、まず `1` から `4` で確認したいパートを選び、`t` で音が鳴るかを見ると分かりやすいです。複数パートをまとめて受信したいときは `a` を押します。

## 画面の見方

左側には Serial ポート一覧が表示されます。未接続のときは、ここをクリックして接続します。

右側の `Status` には、現在の状態が表示されます。

| 表示 | 意味 |
| --- | --- |
| `state` | `PortSelect`、`Ready`、`Playing`、`Error` などの現在状態 |
| `expected` | 今受け付ける予定のパート |
| `muted` | ミュート中かどうか |
| `voices` | 現在鳴っている音の数 |
| `received` | 正常に受け取った NOTE の数 |
| `dropped` | 捨てた NOTE の数 |
| `wrong part` | 期待したパートと違って捨てた数 |
| `last seq` | 最後に受け取ったシーケンス番号 |
| `missing` | 抜けた可能性があるシーケンス数 |
...
| `02_ConstantsAndParts.pde` | 通信仕様、パートID、音量、音程変換などの定数をまとめる |
| `03_NoteProtocol.pde` | 20 byte NOTE フレームを Java/Processing の値に変換する |
| `04_SerialNoteFrameReader.pde` | シリアルのバイト列から、正しい NOTE フレームを切り出す |
| `05_SoundPartManager.pde` | NOTE を受け取り、どの種類の音を鳴らすか決める |
| `06_SoundVoices.pde` | 実際に1つの音を鳴らすクラス群 |
| `07_ScreenView.pde` | 画面表示を担当する |
| `08_EventLogger.pde` | 受信内容やイベントを CSV に保存する |
| `sketch.properties` | Processing のスケッチ設定 |
| `09_ProgramExplanation.html` | プログラム解説用の HTML |
...
`keyPressed()` にはキー操作がまとまっています。`1` から `4` のパート切り替え、`a` の全パート受信、`m` のミュート、`t` のテスト音、`g` の疑似 NOTE などはここで処理されます。

### `02_ConstantsAndParts.pde`
...
起動すると、`orchestra_processing_final_log_年月日_時分秒.csv` という名前の CSV ファイルを作ります。NOTE を受け取ったとき、エラーで捨てたとき、Serial に接続したときなどにログを書き込みます。
...
4. `03_NoteProtocol.pde` の `decodeNote()` が `NotePacket` に変換する。
...
4つのパートIDは `02_ConstantsAndParts.pde` にあります。
...
音色の分岐は `05_SoundPartManager.pde` の `noteOn()` にあります。
...
このため、現状では3つの金管パートは同じ `BrassVoice` を使います。金管音の波形は `05_SoundPartManager.pde` の `brassWaveform` で作っています。

金管音の立ち上がりや消え方は `06_SoundVoices.pde` の `BrassVoice` にあります。リズム音の周波数、波形、短さは `06_SoundVoices.pde` の `RhythmVoice` にあります。

## よく変更する場所

| やりたいこと | 見るファイル |
| --- | --- |
| Serial 通信速度を変えたい | `02_ConstantsAndParts.pde` の `SERIAL_BAUD` |
| パートIDを変えたい | `02_ConstantsAndParts.pde` の `PART_BRASS_1` など |
| 全体音量を変えたい | `02_ConstantsAndParts.pde` の `MASTER_GAIN` |
| 同時に鳴る音数を変えたい | `02_ConstantsAndParts.pde` の `MAX_VOICES_PER_PART` |
| 金管の音色を変えたい | `05_SoundPartManager.pde` の `brassWaveform`、`06_SoundVoices.pde` の `BrassVoice` |
| リズム音を変えたい | `06_SoundVoices.pde` の `RhythmVoice` |
| キー操作を増やしたい | `processing.pde` の `keyPressed()` |
| 画面表示を変えたい | `07_ScreenView.pde` |
| NOTE フレーム仕様を変えたい | `03_NoteProtocol.pde` と `02_ConstantsAndParts.pde` |
...
2. `02_ConstantsAndParts.pde` で定数とパートIDを見る
3. `processing.pde` で全体の流れを見る
4. `03_NoteProtocol.pde` で NOTE フレームの中身を見る
5. `04_SerialNoteFrameReader.pde` で受信処理を見る
6. `05_SoundPartManager.pde` で音色の分岐を見る
7. `06_SoundVoices.pde` で実際の音の作り方を見る
8. `07_ScreenView.pde` と `08_EventLogger.pde` で表示とログを見る
