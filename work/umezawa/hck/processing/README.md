...
1. Processing 4 を開く。
2. `orchestra_processing_final.pde` を開く。
...
| ファイル | 役割 |
| --- | --- |
| `orchestra_processing_final.pde` | メイン処理。初期化、描画、シリアル受信、キー操作、全体の流れを担当 |
| `Config.pde` | 通信仕様、パートID、音量、音程変換などの定数をまとめる |
| `PartManager.pde` | NOTE を受け取り、どの種類の音を鳴らすか決める |
| `Voice.pde` | 実際に1つの音を鳴らすクラス群 |
| `Protocol.pde` | 20 byte NOTE フレームを Java/Processing の値に変換する |
| `SerialFrameReader.pde` | シリアルのバイト列から、正しい NOTE フレームを切り出す |
| `UiView.pde` | 画面表示を担当する |
| `Logger.pde` | 受信内容やイベントを CSV に保存する |
| `sketch.properties` | Processing のスケッチ設定 |
| `program_explanation.html` | プログラム解説用の HTML |

## 各プログラムの詳しい解説

### `orchestra_processing_final.pde`
...
4. `Protocol.pde` の `decodeNote()` が `NotePacket` に変換する。
...
音色の分岐は `PartManager.pde` の `noteOn()` にあります。
...
このため、現状では3つの金管パートは同じ `BrassVoice` を使います。金管音の波形は `PartManager.pde` の `brassWaveform` で作っています。

金管音の立ち上がりや消え方は `Voice.pde` の `BrassVoice` にあります。リズム音の周波数、波形、短さは `Voice.pde` の `RhythmVoice` にあります。

## よく変更する場所

| やりたいこと | 見るファイル |
| --- | --- |
| Serial 通信速度を変えたい | `Config.pde` の `SERIAL_BAUD` |
| パートIDを変えたい | `Config.pde` の `PART_BRASS_1` など |
| 全体音量を変えたい | `Config.pde` の `MASTER_GAIN` |
| 同時に鳴る音数を変えたい | `Config.pde` の `MAX_VOICES_PER_PART` |
| 金管の音色を変えたい | `PartManager.pde` の `brassWaveform`、`Voice.pde` の `BrassVoice` |
| リズム音を変えたい | `Voice.pde` の `RhythmVoice` |
| キー操作を増やしたい | `orchestra_processing_final.pde` の `keyPressed()` |
| 画面表示を変えたい | `UiView.pde` |
| NOTE フレーム仕様を変えたい | `Protocol.pde` と `Config.pde` |
...
2. `Config.pde` で定数とパートIDを見る
3. `orchestra_processing_final.pde` で全体の流れを見る
4. `Protocol.pde` で NOTE フレームの中身を見る
5. `SerialFrameReader.pde` で受信処理を見る
6. `PartManager.pde` で音色の分岐を見る
7. `Voice.pde` で実際の音の作り方を見る
8. `UiView.pde` と `Logger.pde` で表示とログを見る
