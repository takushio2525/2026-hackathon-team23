...
Arduino に組み込む前に、金管4パートの音階、輪唱タイミング、低音伴奏、
...
| 曲 | かえるのうた（ハ長調） |
| テンポ | 固定 96 BPM |
| パート | トランペット / ホルン / トロンボーン（主旋律の輪唱）、チューバ（低音伴奏） |
| 入り拍 | 主旋律 = 0 / 8 / 16 拍、低音 = 0 拍 |
...
はチューバ担当ノードの `kScore[]` に転記できる粒度で作っています。
主旋律3声は同じ配列を利用し、`startBeatNo` だけを `0`, `8`, `16` に
差し替えます。チューバは `startBeatNo = 0` で40拍の低音配列を使用します。
