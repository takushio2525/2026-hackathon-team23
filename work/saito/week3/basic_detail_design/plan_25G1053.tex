...
\begin{itemize}
  ...
  \item 5つの楽器ノードは共通処理を使い，\code{partId}，\code{startBeatNo}，楽譜配列だけを差し替える．
\end{itemize}
...
F3.5 & パート固有ロジック &
\code{partId}，\code{startBeatNo}，楽譜配列をノードごとに差し替え，3声の輪唱，低音，リズムを構成する．\\
F4.1 & NOTEパケット生成 &
...
\node[small, above right=8mm and 13mm of wifi] (n1) {node\_01\\金管1};
\node[small, above right=1mm and 13mm of wifi] (n2) {node\_02\\金管2};
\node[small, below right=6mm and 13mm of wifi] (n3) {node\_03\\金管3};
\node[small, below=13mm of n3] (n4) {node\_04\\低音};
...
5パートの割り当てを表\ref{tab:part-map}に示す．
...
\midrule
node\_01 & \code{0x01} & 金管1 & 0拍 & 主旋律を曲頭から演奏する．\\
node\_02 & \code{0x02} & 金管2 & 4拍 & 主旋律を4拍遅れて演奏する．\\
node\_03 & \code{0x03} & 金管3 & 8拍 & 主旋律を8拍遅れて演奏する．\\
node\_04 & \code{0x04} & 低音 & 0拍 & 拍の頭を支える低音パートを演奏する．\\
...
\node[box, right=of n1] (n) {node\_01〜05\\楽譜配列 + NOTE生成};
...
\midrule
node\_01 & \code{0x01} & 0 & 金管 & 主旋律A \\
node\_02 & \code{0x02} & 4 & 金管 & 主旋律A \\
node\_03 & \code{0x03} & 8 & 金管 & 主旋律A \\
node\_04 & \code{0x04} & 0 & 低音 & 低音伴奏 \\
...
ADR-0004 &
旧方針では指揮1台，楽器4台を想定． &
2026年5月13日の話し合い結果に合わせ，Arduino 5台をすべて楽器ノードとし，指揮棒は別マイコンへ分離する．\\
ADR-0007 &
指揮テンポ変更に演奏速度が追従する． &
楽譜を拍単位で持ち，指揮棒用マイコンから届くBPMから実時間へ変換する．\\
\code{arduino\_塩澤.pdf} &
\code{ScoreEvent}，\code{startBeatNo}，20バイトNOTEを定義． &
同じデータ構造を採用し，対象ノードを\code{node\_01}〜\code{node\_05}の5台へ拡張する．\\
\bottomrule
\end{longtable}
