...
bool     sNavNeedsRelease = false;       // 横振り発火後、dynNorm < NAV_RELEASE_G で解除されるまで再発火を禁止
...
bool updateNav(SystemData& data, uint32_t now, uint8_t itemCount) {
    ...
    if (sNavGate == NavGate::Idle) {
        // 横振り発火後、ユーザーが振りを止める (dynNorm < release) まで再突入を禁止。
        // これにより 1 回の物理的な振りから 1 回だけ発火し、LPF 尾引きや
        // 強制 Idle→即再 Armed で同じ振り中に 2 回トグルする問題を防ぐ。
        if (sNavNeedsRelease) {
            if (data.imu.dynNorm < NAV_RELEASE_G) sNavNeedsRelease = false;
            return false;
        }
    ...
    if (!sNavFired && (windowDone || release)) {
        ...
        if (sNavVertAccum >= sNavHorizAccum * NAV_VERT_DOMINANCE) {
            ...
            // 横振りが支配的 → カーソル移動
            if (itemCount == 2) {
                // 2 項目の場合は方向に依存せずトグル。左右どちらに振っても
                // 毎回モードが切り替わる。方向判定は重力分解の精度や取り付け
                // 角度の影響で不安定になりやすく、端に張り付いて動かなくなる
                // 問題があったため廃止。
                data.game.navCursor = 1 - data.game.navCursor;
                sNavNeedsRelease = true;
            } else if (itemCount > 2) {
                int dom = 0;
                for (int i = 1; i < 3; ++i) {
