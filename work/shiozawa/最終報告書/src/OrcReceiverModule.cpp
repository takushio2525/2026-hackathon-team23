...
//
// 設計意図 (tools/verification/results/MOP5_systematic_shift_analysis_20260710.md Sec.4/Sec.8 案4):
...
void OrcReceiverModule::updateInput(SystemData& data) {
    ...
    // 窓位置を確定するための計測 (MOP5_countermeasure_eval_20260710.md Sec.3.3/Sec.5(b)-2)。
    // 形式: M45S,<partId>,<deviceMs>,<gapMs>  (deviceMs = ストール明け = 今回呼び出しの millis())
    // 閾値 10ms: 名目 2ms + 実測の健常ループ粒度 (発火 late p95 9ms) を上回る異常だけを拾い、
    // シリアル帯域を守る (ストールは ~5 回/秒 なので 1 行 ~25B x 5 = 帯域の 1% 未満)。
    {
