...
public:
    ...
    // 基準時刻をセット（offsetMs を渡すとその ms 過去を起点にできる）
    void setTime(unsigned long offsetMs = 0) {
        _startTime = millis() - offsetMs;
    }
    // 基準時刻からの経過時間 (ms)
    unsigned long getNowTime() const { return millis() - _startTime; }
private:
...
\begin{lstlisting}
enum class WifiMode : uint8_t { SoftAp, Sta };

struct OrcNetConfig {
    WifiMode    mode;                  // node_01: SoftAp / node_02-05: Sta
    const char* ssid;                  // SoftAp なら自身が公開する SSID
    const char* pass;                  // 共通（WPA2-PSK）
    IPAddress   multicastIp;           // 例: IPAddress(239, 0, 0, 1)
    uint16_t    udpPort;               // CTRL/BEAT 共通ポート（5001）
    uint8_t     channel = 6;           // SoftAp 時のみ参照
    uint32_t    reconnectIntervalMs = 2000;
};

struct OrcNetData {
    // 受信結果（OrcReceiverModule が読む）
    ...
    // 送信要求（OrcSenderModule が書き，OrcNetModule.updateOutput() がフラッシュ）
    bool       hasPendingCtrl = false;
    CtrlPacket pendingCtrl    = {};
    bool       hasPendingBeat = false;
    BeatPacket pendingBeat    = {};
    uint8_t    pendingBeatRedundancy = 1;   // 同一 BEAT を何発送るか
};

class OrcNetModule : public IModule {
public:
    explicit OrcNetModule(const OrcNetConfig& config);
    bool init() override;                          // SoftAp: WiFi.beginAP() / Sta: WiFi.begin() + Udp.beginMulticast()
    void updateInput(SystemData& data) override;   // WiFi 状態 + 受信ポーリング
    void updateOutput(SystemData& data) override;  // hasPendingCtrl/Beat を見て送信
...
共通ヘッダの \texttt{timestampMs} を「送信時のマスタ時刻」と読み替え，楽器は受信のたびに
次の更新を行う：

\begin{lstlisting}
// 受信時（CTRL/BEAT どちらでも）
int32_t sample = (int32_t)pkt.header.timestampMs - (int32_t)millis();
offset = offset + alpha * (sample - offset);   // EMA, alpha = 0.10
// 発音判定（applyPattern 内）
uint32_t masterNow = millis() + offset;
if (masterNow >= pendingBeat.playAtMasterMs) firePendingBeat();
\end{lstlisting}
...
// 共有バスピン（XIAO ESP32-S3 Sense の I2C 既定）
constexpr int I2C_SDA_PIN = 5;   // D4 = GPIO5
constexpr int I2C_SCL_PIN = 6;   // D5 = GPIO6

const ImuConfig IMU_CONFIG = {
    .address          = 0x68,    // MPU6050（AD0=GND）。AD0=VCC なら 0x69
    .sampleIntervalMs = 5,       // 200 Hz
    .accelRangeG      = 4,       // MPU6050 は 2/4/8/16 g 選択可
    .gyroRangeDps     = 2000,    // MPU6050 は 250/500/1000/2000 dps 選択可
};
const OrcNetConfig ORC_NET_CONFIG = {
    .mode        = WifiMode::SoftAp,         // node_01 自身が AP を起動
    .ssid        = "OrchestraAP",
    .pass        = "orchestra2026",
...
const OrcSenderConfig ORC_SENDER_CONFIG = {
    ...
    .beatRedundancy    = 2,      // 同一 BEAT を何発連送するか（lookahead 期限切れ回避）
    .beatLookaheadMs   = 50,     // マスタ時刻で何 ms 先を発音指定にするか（穴3対策）
};
...
// SystemData 経由で OrcNetModule に送信を依頼する（直接呼び出しは EMA で禁止）
void OrcSenderModule::updateOutput(SystemData& data) {
    const uint32_t masterNow = millis();   // 指揮者は自分の millis() がマスタ時刻
    // BEAT: イベント駆動（未来時刻指定）
    if (data.beat.event) {
        BeatPacket p;
        p.header.type        = TYPE_BEAT;
        p.header.seq         = ++data.sender.beatSeq;
        p.header.timestampMs = masterNow;
        p.beatNo             = data.beat.beatNo;
        p.playAtMasterMs     = masterNow + _config.beatLookaheadMs;
        // OrcNetModule はこのフラグを見てブロードキャストする
        data.orcNet.pendingBeat            = p;
        data.orcNet.pendingBeatRedundancy  = _config.beatRedundancy;
        data.orcNet.hasPendingBeat         = true;
    }
    // CTRL: 周期駆動（20 Hz）— 楽器側はこの timestampMs でオフセットを学習
...
const OrcNetConfig ORC_NET_CONFIG = {
    .mode        = WifiMode::Sta,            // node_01 SoftAP に接続
    .ssid        = "OrchestraAP",
    .pass        = "orchestra2026",
...
const OrcReceiverConfig ORC_RECEIVER_CONFIG = {
    ...
    .clockSyncEmaAlpha   = 0.10f,       // 時計オフセット EMA 係数（穴1対策）
    .clockSyncMinSamples = 5,           // WaitStart 解除に必要な CTRL 受信数（穴1対策）
    .expiredGraceMs      = 100,         // 過去 BEAT を即発音で許容する範囲（穴3対策）
    .loopIntervalMs      = 5,           // ループ周期．発音判定粒度の下限（穴2対策）
};
...
\begin{lstlisting}[caption={node\_02--05/include/SystemData.h（抜粋）}]
enum class PerformerState : uint8_t {
    Idle = 0, WaitStart = 1, Playing = 2, SelfRun = 3,
};

struct SyncLogicData {
    int32_t  offsetMs     = 0;       // master_now = millis() + offsetMs
    uint16_t sampleCount  = 0;       // WaitStart 解除判定（>= clockSyncMinSamples）
...
struct ReceiverLogicData {
    uint16_t      lastBeatNo     = 0;     // beat_no 重複排除
    uint32_t      lastBeatMs     = 0;     // SelfRun 判定
    PendingBeat   pending;                // マスタ時刻待ち BEAT
    bool          firedThisCycle = false; // 楽譜進行フラグ
};
\end{lstlisting}
