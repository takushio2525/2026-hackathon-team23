...
\begin{tikzpicture}[
  ...
  \draw[arr] (idle)  --                node[lbl]              {WiFi 接続完了\\+ 時刻同期収束}      (wait);
  \draw[arr] (wait)  --                node[lbl]              {beatNo\\$\geq$ startBeatNo}        (play);
...
\texttt{beatLookaheadMs} & \texttt{50}  & マスタ時刻で何 ms 先を発音指定にするか（穴 3 対策） \\
\bottomrule
\end{tabular}
\end{apibox}
...
\texttt{clockSyncEmaAlpha}   & \texttt{0.10}  & 時計オフセット EMA 係数（穴 1 対策） \\
\texttt{clockSyncMinSamples} & \texttt{5}     & WaitStart 解除に必要な CTRL 受信数（穴 1 対策） \\
\texttt{expiredGraceMs}      & \texttt{100}   & 過去 BEAT を即発音で許容する範囲（穴 3 対策） \\
\texttt{loopIntervalMs}      & \texttt{5}     & ループ周期．発音判定粒度の下限（穴 2 対策） \\
\bottomrule
\end{tabular}
\end{apibox}
...
\begin{enumerate}
  ...
  \item \textbf{演奏状態遷移}: \texttt{Idle} $\to$ \texttt{WaitStart}（WiFi 接続完了）$\to$
        \textbf{時計同期収束}（\texttt{sampleCount $\geq$ clockSyncMinSamples}）$\to$
  ...
  \item \textbf{マスタ時刻判定（穴2対策の発音粒度）}: \texttt{masterNow = millis() + offsetMs}
