...
\midrule
\endhead
MOP-1 & 楽器間 同期誤差（4 ノードの発音タイミング差）          & $\leq$ 30 ms \\
...
\begin{description}
  \item[\textmd{\textbf{MOP-1}}（楽器間 同期誤差 $\leq$ 30 ms）]
        合奏のタイミング知覚に関する解説論文 \cite{kawase2021ensemble} では，
        Rasch (1979) や Keller \& Appel (2010) を引きつつ
        「演奏者どうしのズレが\textbf{数 10 ms 程度ならば，演奏者・聴き手とも
        ズレているとは感じない}」とまとめられており，逆に
        \textbf{100 ms を超える}と合っていないと判断される．
        音速 340 m/s に基づく換算では 30 ms は約 10 m の距離に相当し，
        遠隔セッションサービス NETDUETTO の許容遅延 30 ms 以下という運用基準
        \cite{harmonic2020} とも整合する．
        本システムは同一 SoftAP 配下の LAN 内通信であり，物理距離由来の遅延は
        無視できるため，30 ms はやや厳しめだが現実的な目標値として採用する．

  \item[\textmd{\textbf{MOP-2}}（通信遅延 $\leq$ 10 ms）]
        DTM 環境での演奏可能なレイテンシは \textbf{15--25 ms 以内}が快適水準とされる
        \cite{yugo2020latency}．本システムでは BEAT 受信後に楽器ノードが
        \texttt{playAtMasterMs} まで保持して発音するマスタクロック方式を採るため，
        通信遅延そのものは MOP-1 にほぼ吸収される．したがって MOP-2 は
        「DTM 推奨水準のさらに半分以下」を狙った\textbf{設計マージン}として 10 ms を採る．
        音速換算では 10 ms $\approx$ 3.4 m\cite{harmonic2020} で，
        同じ部屋にいる演奏者間の音響遅延に相当する．
        ESP32 + UDP 同 LAN 内通信は数 ms オーダーで到達する報告が多く
        \cite{ipsj_remote2003}，達成可能な範囲である．
  ...
  \item[\textmd{\textbf{MOP-5}}（5 \% パケロス下で聴感破綻なし）]
        家庭用 IP 通信品質の解説 \cite{eo_packet_loss} によれば，
        パケットロス率は 1 \% 以下が良好，3 \% 超で遅延・不具合が顕在化，
        10 \% 超でストレスを感じる水準とされる．
        無線 LAN ではマルチキャストの取りこぼしが有線より有意に多く
        \cite{dena_wifi2018}，本システムは BEAT 連送（\texttt{beatRedundancy}）と
        SelfRun フォールバックでパケロスを吸収する設計とした．
        試験条件としては「家庭用ルータでも実際に観測されうる悪化水準」を擬似する
        意味で 5 \% パケロスを採用する．
  ...
  \item[\textmd{\textbf{MOP-7}}（起動時間 $\leq$ 5 s）]
        ESP32 の Wi-Fi 接続には実測でおよそ 3 秒前後を要する報告
        \cite{zenn_esp32wifi} があり，
        Calibrating（重力オフセット学習，2 秒）と SoftAP 起動を含めても
        5 秒以内に最初の CTRL を送出できる．
        実演開始前のセットアップ時間として
        ユーザーが「いつ振り始めてよいか」を直感的に判断できる範囲は
        およそ 5 秒以内であり，体験面の閾値ともおおむね一致する．

  \item[\textmd{\textbf{MOP-8}}（強弱 3 段階以上）]
        西洋音楽の強弱記号は \cite{wikipedia_dynamics, daxter_dynamics} に示される
        \textbf{p（ピアノ＝弱く）／mf（メゾフォルテ＝やや強く）／f（フォルテ＝強く）}が
        基本単位であり，3 段階あれば多くの初級〜中級曲のダイナミクス指示を
        最低限再現できる．
...
node\_02--05 & WaitStart   & 中速点滅（2 Hz）   & SoftAP 接続済，曲開始 BEAT 待ち \\
...
\begin{tikzpicture}[
  ...
  \draw[arr] (wait)  --                node[lbl]              {時計同期収束\\+ beatNo $\geq$ startBeatNo}        (play);
  \draw[arr] (play)  --                node[lbl, right=1pt]   {BEAT 1500\,ms\\未受信}              (self);
  \draw[arr] (self)  to[bend left=30]  node[lbl, left=1pt]    {実 BEAT 復帰\\(200\,ms 以内)}        (play);
\end{tikzpicture}
...
WaitStart & SoftAP 接続済，時計同期 + 曲頭 BEAT 待ち & CTRL を \texttt{clockSyncMinSamples} 個以上受信し \texttt{offset} が収束するまでロジック無効．収束後 \texttt{beatNo} が \texttt{startBeatNo} に到達するまで待機 \\
...
P1.1            & \textbf{MPU6050（外付け）}   & 1 & 6 軸 IMU（加速度 $\pm$2/4/8/16 g + 角速度 $\pm$250/500/1000/2000 dps）．本案件では $\pm$4 g / $\pm$2000 dps．I2C アドレス \texttt{0x68} \\
...
外部ライブラリ（指揮者）& \texttt{Adafruit\_MPU6050}（または \texttt{I2Cdev/MPU6050}）& I2C 経由で IMU 読み取り \\
...
（50\,ms），LED 点滅周期，BEAT 不応期，SelfRun タイムアウト判定．
\end{apibox}
...
\paragraph{lookahead と期限切れ挙動}\mbox{}\\
...
\begin{itemize}
  ...
  \item \textbf{過去時刻だが \texttt{expiredGraceMs}（仮 100\,ms）以内}: 即発音 + 警告ログ．
        WiFi 詰まりによる遅延を許容する．
  \item \textbf{過去時刻かつ \texttt{expiredGraceMs} を超過}: スキップ + 警告ログ．
        遅すぎる発音はテンポを崩すため．
\end{itemize}

\texttt{beatRedundancy = 2} 以上で同一 BEAT を冗長送信し，どれか 1 発が
\texttt{expiredGraceMs} 内に届けば発火するようにする．

\paragraph{同期確立フェーズ}\mbox{}\\
起動直後の数秒は \texttt{offset} が未収束のため，楽器ノードの \texttt{WaitStart} 状態の
遷移条件を「\textbf{WiFi 接続完了 + CTRL を \texttt{clockSyncMinSamples}（仮 5）個受信}」と
する（\S\ref{sec:basic-state}）．収束前の BEAT は無視するか，受信できても発音判定に
使わない．
...
\texttt{beatAccelThresholdG} & \texttt{1.80}  & 拍検出のノルム閾値（g） \\
\texttt{beatRefractoryMs}    & \texttt{250}   & 拍検出後の不応期 \\
\texttt{bpmEmaAlpha}         & \texttt{0.30}  & テンポ推定 EMA 係数 \\
\texttt{bpmMin}              & \texttt{40.0}  & 推定 BPM 下限クランプ \\
\texttt{bpmMax}              & \texttt{240.0} & 推定 BPM 上限クランプ \\
\texttt{calibrationMs}       & \texttt{2000}  & 起動時キャリブレーション期間 \\
\texttt{reinitRetryMs}       & \texttt{5000}  & init 失敗モジュールの再試行間隔 \\
\bottomrule
\end{tabular}
\end{apibox}
...
\texttt{startBeatNo}         & \texttt{0}     & 輪唱の入り拍（パートごとに差分） \\
\texttt{beatTimeoutMs}       & \texttt{1500}  & SelfRun 発動までの BEAT 途絶許容時間 \\
\texttt{clockSyncEmaAlpha}   & \texttt{0.10}  & 時計オフセット EMA 係数 \\
\texttt{clockSyncMinSamples} & \texttt{5}     & WaitStart 解除に必要な CTRL 受信数 \\
\texttt{expiredGraceMs}      & \texttt{100}   & 過去 BEAT を即発音で許容する範囲 \\
\texttt{loopIntervalMs}      & \texttt{5}     & ループ周期．発音判定粒度の下限 \\
\bottomrule
\end{tabular}
\end{apibox}
...
\begin{apibox}{PerformerState（列挙）}
\begin{tabular}{l p{0.6\textwidth}}
\toprule
\textbf{値} & \textbf{意味} \\
\midrule
\texttt{Idle}      & 起動直後．未初期化 \\
\texttt{WaitStart} & WiFi 接続後，時計同期収束＋曲頭到来を待つ \\
\texttt{Playing}   & 通常演奏．受信 BEAT に追従して楽譜を進める \\
\texttt{SelfRun}   & BEAT 途絶時のフェイルセーフ．直近 BPM で内挿進行 \\
\bottomrule
\end{tabular}
\end{apibox}
...
\texttt{lastBeatMs}     & 直近に受理した自時計時刻．\texttt{beatTimeoutMs} 超過で SelfRun 遷移 \\
\texttt{pending}        & マスタ時刻待ち BEAT（\texttt{PendingBeat}） \\
\texttt{firedThisCycle} & 今周期内に楽譜を進めたか \\
\bottomrule
\end{tabular}
\end{apibox}
...
\textbf{フィールド} & \textbf{役割} \\
\midrule
\texttt{beatAt}     & このイベントを発動する拍番号（\texttt{startBeatNo} 相対） \\
\texttt{noteNumber} & MIDI ノート番号．\texttt{0} なら休符 \\
\texttt{velocity}   & 個別 velocity．CTRL の \texttt{velocity} と乗算合成 \\
\texttt{durationQ8} & 拍の \texttt{1/256} 単位の発音長（\texttt{256 = 1} 拍） \\
...
\texttt{kScore[]}    & \texttt{ScoreEvent} の配列．時系列順に並べる \\
...
\begin{enumerate}
  \item \textbf{時計オフセット更新}: 受信フェーズで \texttt{data.orcNet.hasNewCtrl} or
        ...
        受信回数を \texttt{data.sync.sampleCount} として記録．
  \item \textbf{演奏状態遷移}: \texttt{Idle} $\to$ \texttt{WaitStart}（WiFi 接続完了）$\to$
        \texttt{Playing}（時計同期収束 \texttt{sampleCount $\geq$ clockSyncMinSamples}
        かつ \texttt{lastBeatNo $\geq$ startBeatNo}）$\to$
        \texttt{SelfRun}（\texttt{beatTimeoutMs} 超過）$\to$ \texttt{Playing}（実 BEAT 復帰）．
  \item \textbf{保留 BEAT のキューイング}: 受信した BEAT は \texttt{beatNo} 重複排除のうえ，
        \texttt{(beatNo, playAtMasterMs)} のペアを保留キューに積む．
  \item \textbf{マスタ時刻判定（発音粒度）}: \texttt{masterNow = millis() + offsetMs}
        を計算．保留キューの先頭 BEAT について：
        \begin{itemize}
          \item \texttt{masterNow $\geq$ playAtMasterMs} なら発火．
                \texttt{masterNow - playAtMasterMs $>$ expiredGraceMs} ならスキップ + 警告ログ．
          \item \texttt{masterNow $<$ playAtMasterMs} なら次ループまで待機．
        \end{itemize}
  \item \textbf{仮想 BEAT 内挿}: \texttt{SelfRun} 中は最後の \texttt{currentBpm} から
        \texttt{virtualBeatNo} を進めて発火（マスタ時刻判定はバイパス）．
  \item \textbf{楽譜進行}: 発火 BEAT について \texttt{effectiveBeatNo - startBeatNo} を相対拍として，
...
\texttt{loop()} は \texttt{loopIntervalMs}（仮 5\,ms）で回し，マスタ時刻判定の粒度を
楽器間で揃える．
...
\texttt{applyPattern()} 時計同期  & timestampMs 系列に対し \texttt{offsetMs} が EMA 収束 & 数値列テスト \\
\texttt{applyPattern()} 発音判定  & \texttt{playAtMasterMs} に対し未来/許容過去/超過過去 の3分岐 & 状態機械テスト \\
\bottomrule
\end{longtable}
...
TP-3 & 同期誤差    & MOP-1: $\leq$ 30 ms & 4 楽器が記録した \textbf{発音時のマスタ時刻}（\texttt{masterNow} at NOTE 送出時）を比較．100 拍ぶんログ取得 & 最大差 $\leq$ 30 ms（マスタクロック方式により実機では 5 ms 以下を目標） & W9 (M5) \\
...
設計時点では仮値で進め，実機 T1--T4 結合試験（\S\ref{sec:test-design}）で確定する項目を表 \ref{tab:tbd} に列挙する．
...
\midrule
\endhead
SoftAP マルチキャスト到達性 & 主案: \texttt{239.0.0.1:5001} マルチキャスト & ESP32-S3 Arduino-ESP32 で SoftAP+UDP マルチキャストはネイティブサポートのため到達想定．T2 で実測し，問題があればサブネットブロードキャスト \texttt{192.168.4.255:5001} に切替（\texttt{OrcNetConfig.multicastIp} の差替えのみ） \\
BEAT 連発回数            & \texttt{beatRedundancy = 2}                & T2--T3 のパケロス実測値を見て 1--3 連発に調整 \\
lookahead                & \texttt{beatLookaheadMs = 50}              & T3 で \texttt{expiredGraceMs} 超過率と振り→音の体感遅延を見て 30--80 ms で調整 \\
expiredGraceMs           & \texttt{expiredGraceMs = 100}              & T3 で過去 BEAT のうち即発音許容に倒した発生率を見て調整 \\
時計同期 EMA 係数        & \texttt{clockSyncEmaAlpha = 0.10}          & T2 で \texttt{offsetMs} の収束時間と定常ジッタを見て 0.05--0.20 で調整 \\
WaitStart 解除サンプル数 & \texttt{clockSyncMinSamples = 5}           & T2 で起動から WaitStart 解除までの実時間を見て調整 \\
SelfRun 発動閾値         & \texttt{beatTimeoutMs = 1500}              & T2 でテンポ 60--160 BPM 各レンジでの誤発動率を確認し ±200 ms 単位で調整 \\
拍検出閾値（MPU6050）    & 加速度ノルム閾値 仮 1.80 g                  & TP-1 にて検出率 90\% を満たす値に調整 \\
WiFi チャネル            & \texttt{channel = 6}                       & 会場の電波状況を Site Survey して空きチャネルへ \\
LED 色・点滅周期         & 1/2/5 Hz の点滅                            & 視認性確認後に決定 \\
楽譜データスキーマ       & \texttt{\{beatNo, note, durationBeats\}} 配列 & 楽譜担当との合意で確定 \\
...
\bibitem{kawase2021ensemble}
河瀬諭（相愛大学），
「なぜ合奏は『合う』のだろう？～合奏研究で明らかになった3つのこと～
第1章 合奏は何ミリ秒までならズレてもよい？」，
ON-KEN SCOPE 音楽×研究（公益財団法人ヤマハ音楽振興会），
\url{https://www.yamaha-mf.or.jp/onkenscope/kawasesatoshi1_chapter1/}
（合奏において演奏者間のズレが数 10 ms 程度なら知覚されず，
100 ms を超えるとズレと判断される旨を，Rasch (1979) ほかを引用しつつ解説）．

\bibitem{harmonic2020}
Harmonic-Sound，
「遅延と演奏距離」，
\url{https://harmonic-sound.com/%E9%81%85%E5%BB%B6%E3%81%A8%E6%BC%94%E5%A5%8F%E8%B7%9D%E9%9B%A2/}
（ネットデュエットの許容遅延 30 ms 以下を，音速 340 m/s の換算で
約 10 m 離れた相手とのセッションに相当すると説明）．

\bibitem{yugo2020latency}
Yugo's Music Lab，
「レイテンシーの遅れって何 ms 以内を目指すべき！？演奏に支障が出ない時間はどのくらい？」，
\url{https://yugo-music.jp/article-12445.html}
（DTM 環境での実測に基づき，演奏に支障の出ないレイテンシを 15--25 ms 以内とする目安を提示）．

\bibitem{ipsj_remote2003}
情報処理学会，
...
（パケットロス率の目安として，1 \% 以下=良好，3 \% 超=遅延や不具合，
10 \% 超=ストレスを感じる水準と説明）．
...
（無線 LAN ではマルチキャストの取りこぼしや有線比でのロス増加が
有意に観測されるという実例）．

\bibitem{zenn_esp32wifi}
Zenn 記事（penguininin），
「ESP32 の WiFi 接続速度が遅い」，
\url{https://zenn.dev/penguininin/scraps/95eb01990ae351}
（ESP32 が Wi-Fi アクセスポイントへ接続するまでに 3 秒前後を要すること，
静的 IP・BSSID 指定で短縮可能なことの実測情報）．

\bibitem{wikipedia_dynamics}
ウィキペディア日本語版，
「強弱法」，
\url{https://ja.wikipedia.org/wiki/%E5%BC%B7%E5%BC%B1%E6%B3%95}
（西洋音楽の強弱記号 pp / p / mp / mf / f / ff の体系，および
基本単位としての p / mf / f の位置づけ）．

\bibitem{daxter_dynamics}
Daxter Music，
「強弱記号 — 楽典」，
\url{https://www.daxter-music.jp/wmi/index007.html}
（p, mf, f を中心とした強弱記号の意味と段階関係）．
