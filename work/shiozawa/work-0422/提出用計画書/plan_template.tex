...
これを受信して楽譜を進行させながら NOTE を PC へユニキャストで送信する．
...
\begin{tikzpicture}[
  ...
  \draw[arr] (ap) -- (n4);
  \draw[arr] (ap) -- (n5);
  \draw[arr2] (n2) to[bend left=10] node[above, font=\scriptsize, sloped] {NOTE (UDP/5002 Unicast)} (pc);
  \draw[arr2] (n3) -- (pc);
  \draw[arr2] (n4) -- (pc);
...
(5) node\_02〜05 $\to$ PC: \textbf{NOTE}
   （\texttt{note\_number}, \texttt{velocity}, \texttt{duration\_ms}）をユニキャスト．\\
...
\textbf{ノード間の通信プロトコル IF}（CTRL/BEAT/NOTE）の二層に分けて定義する．
通信プロトコルの基本方針はここで述べ，バイト単位のパケットレイアウトは
...
\midrule
\endhead
CTRL & node\_01 → node\_02--05 (Bcast) & 定期（20 Hz）       & 冪等         & BPM・velocity・state を継続同期 \\
BEAT & node\_01 → node\_02--05 (Bcast) & 拍検出時（不定期） & 即時性重視   & 楽譜ポインタ前進トリガ \\
NOTE & node\_02--05 → PC (Unicast)     & 楽譜イベント発火時 & 順序保証     & PC への発音依頼 \\
\bottomrule
\end{longtable}
...
\subsection{通信方式（WiFi STA + UDP）}

ネットワーク仕様を表 \ref{tab:net-spec} に示す．運用時に変わる値（SSID / パス・PC IP）は
\texttt{OrcNetConfig} および \texttt{NoteSenderConfig} のリテラルで一元管理する．

\begin{longtable}{p{0.26\textwidth} p{0.66\textwidth}}
\caption{ネットワーク仕様}\label{tab:net-spec}\\
\toprule
\textbf{項目} & \textbf{設計} \\
\midrule
\endfirsthead
...
ポート       & node\_01 → node\_02--05: \textbf{UDP/5001}（ブロードキャスト 255.255.255.255）／node\_02--05 → PC: \textbf{UDP/5002}（PC IP へユニキャスト） \\
PC IP        & 仮 \texttt{192.168.4.2} を \texttt{NoteSenderConfig} に直書き．本番で差替え \\
...
5 ノードと PC は同一 LAN（WiFi AP 配下）に接続する．node\_01 は CTRL/BEAT を
ブロードキャスト，node\_02--05 は NOTE を PC へユニキャストで送る．本番運用時の
SSID／パス／PC IP は \texttt{ProjectConfig} のリテラルで切替可能とする．
...
\texttt{NoteSenderModule} & node\_02--05 & 出力       & \texttt{data.noteOut.pendingOn/Off} を見て NOTE 送信キューに投入 \\
\texttt{OrcNetModule}     & 全ノード     & 入出力     & WiFi 接続維持，UDP 受信ポーリング（入力）と送信キューフラッシュ（出力） \\
...
\begin{apibox}{NOTE ペイロード（type = 3，楽譜イベント発火時送信，全 20 バイト）}
\begin{tabular}{l l l p{0.42\textwidth}}
\textbf{オフセット} & \textbf{フィールド} & \textbf{型} & \textbf{説明} \\
\midrule
...
\subsubsection{\texttt{OrcNetModule} API}

WiFi 接続維持と UDP 送受信ポーリングを集約する \texttt{IModule} 実装．入出力両方の責務を
持つため，\texttt{inputModules[]} と \texttt{outputModules[]} の両方に登録する．
