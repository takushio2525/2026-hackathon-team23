...
PC 側 Processing による音色合成・スピーカ出力（梅澤担当）は本章のスコープ外とし，
データ受け渡し境界（NOTE パケット）のみを定義する．本書ではノード間（指揮者 ↔ 楽器）の
通信は WiFi UDP，PC ↔ Arduino 間（楽器 → PC の NOTE 送信）は \textbf{USB Serial}
の二系統に分離した構成を採る．
...
5 ノードと WiFi AP・PC の関係を図 \ref{fig:overview} に示す．指揮者 node\_01 は
ローカル WiFi AP に対して CTRL/BEAT をブロードキャストし，楽器 node\_02〜05 は
これを受信して楽譜を進行させながら NOTE を \textbf{USB Serial}（\texttt{Serial.write}）
で PC へ送信する．Arduino--PC 間は WiFi を介さず，給電と Serial 通信を兼ねた USB 接続で
直結する．
...
\begin{tikzpicture}[
  ...
  node_box/.style={draw=accentblue, fill=codebg, rounded corners=2pt, minimum width=26mm, minimum height=11mm, align=center, font=\small},
  ap_box/.style  ={draw=noteyellow, fill=lightgray, rounded corners=2pt, minimum width=22mm, minimum height=10mm, align=center, font=\small},
  pc_box/.style  ={draw=okgreen, fill=lightgray, rounded corners=2pt, minimum width=30mm, minimum height=11mm, align=center, font=\small},
  arr/.style     ={-{Stealth[length=2.5mm]}, thick, accentblue},
  arr2/.style    ={-{Stealth[length=2.5mm]}, thick, okgreen},
]
  \node[node_box] (n1)                            {node\_01\\指揮者 + IMU};
  \node[ap_box,   right=of n1] (ap)               {WiFi AP};
  \node[node_box, right=22mm of ap, yshift= 18mm] (n2)  {node\_02\\楽器 A};
  \node[node_box, right=22mm of ap, yshift=  6mm] (n3)  {node\_03\\楽器 B};
  \node[node_box, right=22mm of ap, yshift= -6mm] (n4)  {node\_04\\楽器 C};
  \node[node_box, right=22mm of ap, yshift=-18mm] (n5)  {node\_05\\楽器 D};
  \node[pc_box, right=42mm of n3, yshift=-6mm] (pc)     {PC（Processing）\\音色合成・出力};

  \draw[arr] (n1) -- node[above, font=\scriptsize] {CTRL/BEAT (UDP/5001 Bcast)} (ap);
  \draw[arr] (ap) -- (n2);
  \draw[arr] (ap) -- (n3);
  \draw[arr] (ap) -- (n4);
  \draw[arr] (ap) -- (n5);
  \draw[arr2] (n2) to[bend left=10] node[above, font=\scriptsize, sloped] {NOTE (USB Serial)} (pc);
  \draw[arr2] (n3) -- (pc);
  \draw[arr2] (n4) -- (pc);
  \draw[arr2] (n5) to[bend right=10] (pc);
\end{tikzpicture}
\caption{全体ブロック図（指揮者 1 + 楽器 4 + PC）}\label{fig:overview}
\end{figure}
...
node\_02 & 楽器 A             & BEAT 同期で楽譜進行 → NOTE 送出（第 1 声） & 塩澤 \\
node\_03 & 楽器 B             & 同上（第 2 声） & 塩澤 \\
node\_04 & 楽器 C             & 同上（第 3 声） & 塩澤 \\
node\_05 & 楽器 D             & 同上（第 4 声 or リズム） & 塩澤 \\
...
\midrule
\endhead
CTRL & node\_01 → node\_02--05 (WiFi Bcast)   & 定期（20 Hz）       & 冪等         & BPM・velocity・state を継続同期 \\
BEAT & node\_01 → node\_02--05 (WiFi Bcast)   & 拍検出時（不定期） & 即時性重視   & 楽譜ポインタ前進トリガ \\
NOTE & node\_02--05 → PC (USB Serial)         & 楽譜イベント発火時 & 順序保証     & PC への発音依頼 \\
\bottomrule
\end{longtable}
...
\begin{itemize}
  ...
  \item \textbf{楽譜側内挿}: BEAT が一定時間（既定 1500\,ms）来なければ楽器が自走（SelfRun）して
        直前 BPM から拍を内挿する．
  \item \textbf{NOTE は単一経路}: 楽器 → PC で 1 系統．楽譜に従って必ず送出する．
        Arduino--PC 間は USB Serial 直結のため，パケロスやネットワーク輻輳の心配なく
        確定到達するのが利点．
\end{itemize}
...
\midrule
\endhead
ノード間（CTRL/BEAT）    & WiFi UDP．node\_01 → node\_02--05 へ \textbf{UDP/5001} ブロードキャスト（\texttt{255.255.255.255}） \\
...
PC--Arduino 間（NOTE）   & \textbf{USB Serial（CDC, \texttt{Serial.write}）}．node\_02--05 が PC のシリアルポートへ直接書込み．PC の Processing は 4 ポートを並列に開いて受信 \\
...
P1.2 楽器       & Arduino UNO R4 WiFi      & 4 & node\_02--05 同一 H/W \\
P1.2            & 内蔵 WiFi                 & 4 & 受信（指揮者）+ 送信（PC） \\
P1.2            & 状態 LED                  & 4 & \texttt{LED\_BUILTIN} 流用 \\
P2 ネットワーク & ローカル WiFi AP          & 1 & スマホテザリング or 専用ルータ \\
P3 給電・通信   & USB Type-C ケーブル       & 5 & 楽器 4 本は \textbf{PC へ直接接続}（給電 + Serial 通信兼用）．指揮者ノードは USB ハブ経由で給電のみ \\
P4 境界（参考） & PC（Processing）          & 1 & 本書スコープ外．node\_02--05 から USB Serial で NOTE を受信 \\
...
node\_02--05 & 給電 + NOTE 送信    & USB Type-C（PC へ直接接続．\texttt{Serial.write} で NOTE 出力） \\
\bottomrule
\end{longtable}
...
\textbf{Arduino と PC}は USB Serial 直結で通信する．node\_01 は CTRL/BEAT を WiFi で
ブロードキャストし，node\_02--05 は NOTE を PC のシリアルポートへ \texttt{Serial.write}
で書き込む．本番運用時の SSID／パスは \texttt{OrcNetConfig} のリテラルで切替可能とする．
PC 側の Processing はノード台数ぶん（4 ポート）のシリアルを並列に開いて受信する．

\subsection{物理配置}

演奏時は指揮者を中心に楽器 4 台を扇形に配置し，PC とスピーカは観客側からは見えない
位置に置く．給電は USB ハブで集約する．配置に関する具体寸法・治具設計はチーム全体側
（梅澤担当）に委ねる．
...
シリアライズする．CTRL/BEAT は WiFi UDP，NOTE は USB Serial（バイナリ列を直接書込）で
転送する．\texttt{magic = 0x4F52} は WiFi UDP ではプロトコル識別子，USB Serial では
\textbf{フレーム同期マーク}を兼ねる（PC 側は magic を探して同期し，\texttt{type} で
ペイロード長を判定して残りを読む）．
...
struct OrcNetConfig {
    const char* ssid;
    const char* pass;
    uint16_t    listenPort;            // 受信用 UDP ポート（5001）
...
public:
    ...
    bool enqueueCtrl(const CtrlPacket& p);   // ブロードキャスト:5001
    bool enqueueBeat(const BeatPacket& p);   // ブロードキャスト:5001
};
\end{lstlisting}
\end{apibox}
...
struct NoteSenderConfig {
    ...
    uint8_t  partId;                // 0x02..0x05（PC 側でノード識別に使用）
};
...
PC 側 Processing は，各楽器ノードに対応する USB シリアルポートを開き，magic
\texttt{0x4F52} を探してフレーム同期し，\texttt{type=3} のとき NOTE ペイロード 8 バイトを
読み取って音色合成に渡す．\texttt{part\_id} で送信元ノードを識別する．
...
const NoteSenderConfig NOTE_SENDER_CONFIG = {
    .baudRate = 115200,           // USB CDC（PC のシリアルポートへ書込み）
    .partId   = 0x02,             // node_02。他ノードは 0x03/0x04/0x05 に書換え
};
const StatusLedConfig STATUS_LED_CONFIG = {
    .pin             = LED_BUILTIN,
    .blinkIntervalMs = 500,
...
\texttt{Serial.write} で PC のシリアルポートへ一括書込みする（PC 側は magic
\texttt{0x4F52} を探してフレーム同期し，\texttt{part\_id} で送信元ノードを識別する）．
...
\midrule
\endhead
node\_02 & \texttt{0x02} (パート A) & 0          & パート A の楽譜 \\
node\_03 & \texttt{0x03} (パート B) & 4          & パート B（輪唱では A と同旋律も多い） \\
node\_04 & \texttt{0x04} (パート C) & 8          & パート C \\
node\_05 & \texttt{0x05} (パート D) & 12 or 0    & パート D（リズムなら 0） \\
\bottomrule
\end{longtable}
