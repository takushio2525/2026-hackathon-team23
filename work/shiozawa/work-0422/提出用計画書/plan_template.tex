...
\begin{tikzpicture}[
  ...
  ap_box/.style  ={draw=noteyellow, fill=lightgray, rounded corners=2pt, minimum width=28mm, minimum height=12mm, align=center, font=\small},
  ...
  \node[ap_box]                (n1) at ($(n3)!0.5!(n4) + (-40mm,0)$) {node\_01\\XIAO ESP32-S3 Sense\\+ MPU6050 \textbf{（SoftAP）}};
  ...
  \draw[arr] (n1) -- node[above, font=\scriptsize, align=center] {CTRL/BEAT\\(UDP Mcast)} (n2);
  \draw[arr] (n1) -- (n3);
  \draw[arr] (n1) -- (n4);
  \draw[arr] (n1) -- (n5);
...
(3) 振り下ろしを検出: node\_01（SoftAP）$\to$ node\_02〜05 に \textbf{BEAT}
   （\texttt{beat\_no}, \texttt{timestamp\_ms}）を UDP マルチキャスト．\\
...
\begin{itemize}
  ...
  \item \textbf{ジッタ（楽器間同期誤差）}: BEAT 受信時刻ではなく
        \textbf{マスタ時刻 \texttt{play\_at\_master\_ms} で発音タイミングを揃える}
...
\begin{tikzpicture}[
  ...
  \draw[arr] (idle)  --                node[above]      {WiFi 接続完了 + 時刻同期収束}        (wait);
...
WaitStart & SoftAP 接続済，時計同期 + 曲頭 BEAT 待ち & CTRL を \texttt{clockSyncMinSamples} 個以上受信し \texttt{offset} が収束するまでロジック無効．収束後 \texttt{beat\_no} が \texttt{startBeatNo} に到達するまで待機 \\
...
8 & \texttt{timestamp\_ms} & \texttt{uint32\_t} & \textbf{マスタ時刻}（= 指揮者ノードの \texttt{millis()}）．楽器側は受信時刻と比較してオフセット推定に使う（\S\ref{subsec:clock-sync}） \\
\end{tabular}
\end{apibox}

\begin{apibox}{CTRL ペイロード（type = 1，定期 20 Hz 送信，全 20 バイト）}
\begin{tabular}{l l l p{0.42\textwidth}}
\textbf{オフセット} & \textbf{フィールド} & \textbf{型} & \textbf{説明} \\
\midrule
...
\begin{apibox}{BEAT ペイロード（type = 2，拍検出時イベント送信，全 20 バイト）}
\begin{tabular}{l l l p{0.42\textwidth}}
\textbf{オフセット} & \textbf{フィールド} & \textbf{型} & \textbf{説明} \\
\midrule
12 & \texttt{beat\_no}            & \texttt{uint16\_t}   & 曲頭からの拍番号（曲開始時に 0 リセット） \\
14 & 予約                          & \texttt{uint8\_t[2]} & ゼロ埋め \\
16 & \texttt{play\_at\_master\_ms} & \texttt{uint32\_t}   & \textbf{マスタ時刻でこの ms に発音せよ} という未来時刻指定．送信時に \texttt{master\_now + lookahead} で算出 \\
\end{tabular}
\end{apibox}
...
BEAT は「\textbf{マスタ時刻 \texttt{play\_at\_master\_ms} に発音せよ}」という \textbf{未来時刻指定}
の指示として送る．楽器は自分の \texttt{millis()} をマスタ時刻に変換するためのオフセットを
学習し続け，\texttt{master\_now} がその時刻に到達した瞬間に発音する．WiFi の到達時刻が
ばらついても，4 台が見ている共通のマスタ時刻に揃うため発音タイミングは合致する．

\paragraph{オフセット推定（楽器側）}\mbox{}\\
共通ヘッダの \texttt{timestamp\_ms} を「送信時のマスタ時刻」と読み替え，楽器は受信のたびに
次の更新を行う：

\begin{lstlisting}
// 受信時（CTRL/BEAT どちらでも）
int32_t sample = (int32_t)pkt.header.timestamp_ms - (int32_t)millis();
offset = offset + alpha * (sample - offset);   // EMA, alpha = 0.10
// 発音判定（applyPattern 内）
uint32_t master_now = millis() + offset;
if (master_now >= pendingBeat.play_at_master_ms) firePendingBeat();
\end{lstlisting}
...
指揮者は BEAT 送出時に \texttt{play\_at\_master\_ms = master\_now + beatLookaheadMs}
を計算する（\texttt{beatLookaheadMs} は \texttt{ORC\_SENDER\_CONFIG} で集約，仮値 50\,ms）．
楽器側は次の挙動で安全側に倒す：

\begin{itemize}
  \item \textbf{未来時刻}（\texttt{play\_at\_master\_ms > master\_now}）: 通常．\texttt{master\_now} 到達まで待機．
...
\begin{enumerate}
  ...
  \item \textbf{保留 BEAT のキューイング}: 受信した BEAT は \texttt{beat\_no} 重複排除のうえ，
        \texttt{(beatNo, playAtMasterMs)} のペアを保留キューに積む．
  \item \textbf{マスタ時刻判定（穴2対策の発音粒度）}: \texttt{master\_now = millis() + offsetMs}
        を計算．保留キューの先頭 BEAT について：
        \begin{itemize}
          \item \texttt{master\_now $\geq$ playAtMasterMs} なら発火．
                \texttt{master\_now - playAtMasterMs $>$ expiredGraceMs} ならスキップ + 警告ログ
                （穴3対策）．
          \item \texttt{master\_now $<$ playAtMasterMs} なら次ループまで待機．
        \end{itemize}
...
\texttt{applyPattern()} 発音判定  & \texttt{play\_at\_master\_ms} に対し未来/許容過去/超過過去 の3分岐 & 状態機械テスト \\
\bottomrule
\end{longtable}
...
TP-3 & 同期誤差    & MOP-1: $\leq$ 30 ms & 4 楽器が記録した \textbf{発音時のマスタ時刻}（\texttt{master\_now} at NOTE 送出時）を比較．100 拍ぶんログ取得 & 最大差 $\leq$ 30 ms（マスタクロック方式により実機では 5 ms 以下を目標） & W9 (M5) \\
...
\begin{longtable}{l p{0.42\textwidth} p{0.36\textwidth}}
