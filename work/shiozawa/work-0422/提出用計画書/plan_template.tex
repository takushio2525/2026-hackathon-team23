...
\begin{description}
  \item[\textmd{\textbf{MOP-1}}（楽器間 同期誤差 $\leq$ 30 ms）]
        ...
        無視できるため，「数 10 ms」の上端を採って 30 ms を目標値とする．
  ...
  \item[\textmd{\textbf{MOP-2}}（通信遅延 $\leq$ 10 ms）]
        ...
        したがって MOP-2 は MOP-1 の許容範囲（30 ms）の 1/3 程度を割り当てた
  ...
  \item[\textmd{\textbf{MOP-5}}（5 \% パケロス下で聴感破綻なし）]
        ...
        \cite{dena_wifi2018}．
        本システムは BEAT 連送（\texttt{beatRedundancy}）と SelfRun
        フォールバックでパケロスを吸収する設計であり，
