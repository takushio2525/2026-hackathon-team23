...
\begin{description}
  ...
  \item[\textmd{\textbf{MOP-7}}（起動時間 $\leq$ 5 s）]
        ...
        指揮者ノードの Calibrating（重力オフセット学習）に 2 秒を要する想定で，
...
\texttt{calibrationMs}         & \texttt{2000}  & 起動時キャリブレーション期間（重力ベクトルの平均化） \\
...
\texttt{Calibrating} & 重力オフセット採取中（起動から \texttt{calibrationMs}） \\
...
\texttt{CalibrationData}    & \texttt{done}（完了フラグ）／\texttt{startMs}（採取開始時刻）／\texttt{gravityOffset[3]}（重力ベクトルのオフセット） \\
...
\begin{enumerate}
  \item \textbf{IIR LPF + ノルム計算 + 動加速度算出}: \texttt{data.imu.acc} を一次ローパス
        （$\alpha = 0.10$）で平滑化し \texttt{accLpf} を得る．
        \texttt{dynAcc[i] = accLpf[i] - calibration.gravityOffset[i]} で重力ベクトルを差し引き，
        \texttt{dynNorm = |dynAcc|} を拍検出の判定対象とする．
        \texttt{Calibrating} 完了前は \texttt{gravityOffset = 0} のため \texttt{dynNorm $\approx$ accNorm}．
...
ただし起動時 \texttt{Calibrating} の 2 秒間に静止していないと重力ベクトル推定がずれるため，使用者には「起動後 2 秒間は腕を自然に下ろす」だけを案内する．
