...
ボーレート               & 115200 bps．USB CDC ではボーレート設定は実質無視されるが，両端で値を揃えておく．\texttt{NoteSenderConfig.baudRate} で集約 \\
...
\begin{itemize}
  ...
  \item \textbf{BEAT 取りこぼし}: 同一 BEAT を \texttt{beatRedundancy} 回（既定 2）連送し，楽器側で
        \texttt{beatNo} の重複を排除する．BEAT が長く来なくても楽器は \texttt{Playing} に留まり，
  ...
  \item \textbf{ジッタ（楽器間同期誤差）}: BEAT 受信時刻ではなく
        ...
        誤差に押し付けられ，4 台の発音タイミングはほぼ揃う．ただし「鳴らない」を最優先して
        過去時刻の BEAT も即発火するため，ある 1 台だけ受信が大きく遅れた場合はその台の発音が
        一時的にずれる（厳密な台間同期は保証しない．\S\ref{subsec:clock-sync}・TP-3）．
...
P4 境界（参考） & Mac（Processing）         & 4 & 本書スコープ外．各楽器から USB Serial で NOTE を受信し，倍音・ADSR 調整・波形生成を行う．楽器ごとに 1 台を前提とするが，揃わない場合は 1 台で 4 ポートを受ける構成にフォールバックできる \\
...
を探して同期し，固定 20 バイトを 1 パケットとして読む．\texttt{type} で CTRL/BEAT/NOTE を
...
\textbf{オフセット} & \textbf{フィールド} & \textbf{型} & \textbf{説明} \\
\midrule
12 & \texttt{partId}      & \texttt{uint8\_t}    & \texttt{0x02}--\texttt{0x05}（= node\_02--05．どの楽器／声部かを表す楽器番号） \\
...
\textbf{フィールド} & \textbf{役割} \\
\midrule
\texttt{baudRate} & USB CDC ボーレート（既定 115200．CDC では実質無視されるが両端で揃える） \\
...
ただし各台の内部経路のジッタ分布が大きく異なるとこの前提は崩れるため，4 台の同期誤差は実機で確認し
（TP-3），必要なら \texttt{beatRedundancy} や \texttt{beatLookaheadMs} を調整する．
...
この方式は「鳴らない」を最優先した best-effort 同期であり，ある 1 台だけ受信が \texttt{lookahead}（50\,ms）を
大きく超えて遅れた場合，その台の発音は一時的にずれる．通常はどれか 1 発が \texttt{lookahead} 内に届くため
聴感上は揃うが，厳密な台間同期は保証しない．TP-3 で実測し，許容範囲に収まらなければ \texttt{lookahead} を
広げる・連送回数を増やす等で調整する．
...
\texttt{startBeatNo}         & \texttt{0}     & 輪唱の入り拍．主旋律 1 番（node\_02）は \texttt{0}，2 番・3 番は曲頭から数拍ずらして入る（具体値は楽譜確定時に決める） \\
...
\texttt{partId}・\texttt{startBeatNo}・楽譜配列のみ（表 \ref{tab:performer-diff}）．輪唱は \texttt{startBeatNo} を
...
\toprule
\textbf{ノード} & \textbf{partId} & \textbf{startBeatNo} & \textbf{score\_data} \\
\midrule
\endfirsthead
\toprule
\textbf{ノード} & \textbf{partId} & \textbf{startBeatNo} & \textbf{score\_data} \\
...
進行は \texttt{currentEventIndex} 駆動なので発火条件には使わない．
...
\begin{longtable}{p{0.06\textwidth} p{0.11\textwidth} p{0.10\textwidth} p{0.30\textwidth} p{0.18\textwidth} p{0.06\textwidth}}
\caption{試験項目（「段階」は \ref{tab:integration-tier} の結合テスト Tier に対応）}\label{tab:tpm-list}\\
\toprule
\textbf{ID} & \textbf{項目} & \textbf{目標} & \textbf{手順} & \textbf{合否基準} & \textbf{段階} \\
\midrule
\endfirsthead
\toprule
\textbf{ID} & \textbf{項目} & \textbf{目標} & \textbf{手順} & \textbf{合否基準} & \textbf{段階} \\
\midrule
\endhead
TP-1 & 拍検出精度  & $\geq$ 90\%   & 80/120/160 BPM のメトロノームに合わせて指揮し，30 秒の検出数 vs 実拍数 & $\geq$ 90\% & T1 \\
TP-2 & 通信遅延    & $\leq$ 10 ms  & 指揮者が BEAT を送出 → 楽器が即 ACK を返す → 指揮者で受信，の往復時間を node\_01 の \texttt{millis()} で 100 サンプル計測し片道を概算（時計同期不要．空中区間と楽器側内部経路を含む） & 片道概算 平均 $\leq$ 10 ms & T2 \\
TP-3 & 同期誤差    & $\leq$ 20 ms  & 4 楽器が記録した \textbf{発音時のマスタ時刻}（NOTE 送出時の \texttt{masterNow}）を比較．100 拍ぶんログ取得 & 最大差 $\leq$ 20 ms（発音時刻は \texttt{millis()}（1 ms 粒度）で記録するため，これより細かい分解能は扱わない．方式上は 20 ms より小さく収まる見込み） & T3 \\
TP-4 & テンポ追従  & $\leq$ 2 拍   & 80 → 120 BPM 切替時に楽器側 BPM が 120 に入るまでの拍数 & $\leq$ 2 拍 & T3 \\
TP-5 & パケロス耐性 & 5\% パケロスで継続 & 楽器側で受信 BEAT をランダムに 5\% 破棄するテストビルドで演奏し，破綻なきこと & 聴感破綻なし & T3 \\
TP-6 & CPU 負荷    & $\leq$ 2 ms   & \texttt{micros()} で入力フェーズ所要時間を 10 分ログ取得（楽器ノードと指揮者ノード node\_01 の両方で実施） & 最大 $\leq$ 2 ms & T3 \\
TP-7 & 起動時間    & $\leq$ 5 s    & 電源投入 → 最初の CTRL 送出までを 5 回平均 & $\leq$ 5 s & T1 \\
TP-8 & 強弱制御（ストレッチ）& 3 段階分離 & 小・中・大の振り幅で velocity 出力値を記録（ストレッチ実装後） & 3 段階分離 & T4 \\
\bottomrule
\end{longtable}
...
設計値のうち実機の結合試験（\S\ref{sec:test-design} の T1--T4）で詰めるもの，および楽譜・音色の
仕様確定を待って実装するものを表 \ref{tab:tbd} に列挙する．
...
\midrule
\endhead
BEAT 連発回数            & \texttt{beatRedundancy = 2}                & T3 のパケロス・同期誤差を見て 1--3 連発で調整 \\
lookahead                & \texttt{beatLookaheadMs = 50}              & T3 で振り→音の体感遅延と，最も遠い楽器が \texttt{waitMs $\leq 0$} になる比率を見て 30--80 ms で調整 \\
...
強弱推定（ストレッチ）    & 振り幅 → velocity 線形写像の 3 段階（現状は CTRL.velocity = 64 固定でスタブ） & TP-8 で p / mf / f が分離して聞こえる写像式に調整．必達機能の完成後に着手 \\
ビブラート（ストレッチ）  & 角速度の高周波成分から振幅変調（仮）       & 必達機能の完成後に着手．Mac 側でフィルタ／LFO を適用 \\
ドラム node\_05         & 金管同様の \texttt{ScoreEvent} で実装（仮） & 主旋律 3 パートの完成後に着手．\texttt{noteNumber} は GM ドラムマップ準拠を想定 \\
楽譜データスキーマ       & \texttt{ScoreEvent}（本書の定義）          & 楽譜エディタ（齋藤担当）の \{pitch, durationBeats\} 形式 → \texttt{ScoreEvent} 配列を生成する変換層で吸収．変換層の所在・担当・期限は楽譜仕様の確定時に決める \\
