...
そのまま発音に使うと楽器間同期誤差が運次第になる．本設計ではこのジッタを
...
同じ値だけずれるので楽器間相対同期には影響しない（楽器間同期誤差で問題になるのはこの相対同期である）．
...
将来，楽器間同期誤差の評価で「過去 BEAT の連発が誤差として無視できない」ことが分かれば，
...
楽器間同期誤差の評価は同期収束以降の定常状態で行う．\texttt{clockSyncMinSamples} はデバッグ表示用フラグに格下げ．
...
テスト実装では「3 台同時に同じ拍を踏むか」を楽器間同期の検証用に試したかったため \texttt{startBeatNo}
...
T3 & node\_01 + node\_02--05 & 4 台がほぼ同時に NOTE．同期誤差 $\leq$ 20 ms                            & USB ハブ + ログ集約 \\
...
設計目標値ごとの試験項目を表 \ref{tab:tpm-list} に示す．
各試験の「目標」「合否基準」欄の数値は，演奏体験として破綻しない水準と，
...
\caption{試験項目}\label{tab:tpm-list}\\
\toprule
...
\midrule
\endhead
TP-1 & 拍検出精度  & $\geq$ 90\%   & 80/120/160 BPM のメトロノームに合わせて指揮し，30 秒の検出 vs 実拍数 & $\geq$ 90\% & W6 (M3) \\
TP-2 & 通信遅延    & $\leq$ 10 ms  & node\_01 が BEAT を送信した直後に同 BEAT を自身でループバック受信し，送信〜受信の時刻差を node\_01 単体の \texttt{millis()} で 100 サンプル計測（時計同期不要） & 平均 $\leq$ 10 ms & W7 (M4) \\
TP-3 & 同期誤差    & $\leq$ 20 ms  & 4 楽器が記録した \textbf{発音時のマスタ時刻}（\texttt{masterNow} at NOTE 送出時）を比較．100 拍ぶんログ取得 & 最大差 $\leq$ 20 ms（マスタクロック方式により実機では 5 ms 以下を目標） & W9 (M5) \\
TP-4 & テンポ追従  & $\leq$ 2 拍   & 80 → 120 BPM 切替時に楽器側 BPM が 120 に入るまでの拍数 & $\leq$ 2 拍 & W9 (M5) \\
TP-5 & パケロス耐性 & 5\% パケロスで継続 & ルータで擬似 5\% パケロスを設定，演奏破綻なきこと & 聴感破綻なし & W11 (M7) \\
TP-6 & CPU 負荷    & $\leq$ 2 ms   & \texttt{micros()} で入力フェーズ所要時間を 10 分ログ取得 & 最大 $\leq$ 2 ms & W11 (M7) \\
TP-7 & 起動時間    & $\leq$ 5 s    & 電源投入 → 最初の CTRL 送出までを 5 回平均 & $\leq$ 5 s & W11 (M7) \\
TP-8 & 強弱制御（ストレッチ）& 3 段階分離 & 小・中・大の振り幅で velocity 出力値を記録 & 3 段階分離 & W10 (M6) \\
\bottomrule
\end{longtable}

\subsection{要求 $\to$ 検証の対応}

\begin{longtable}{l p{0.34\textwidth} p{0.44\textwidth}}
\caption{要求と検証の対応表}\label{tab:req-verify}\\
\toprule
\textbf{要求 ID} & \textbf{要求内容} & \textbf{検証方法} \\
\midrule
\endfirsthead
\toprule
\textbf{要求 ID} & \textbf{要求内容} & \textbf{検証方法} \\
\midrule
\endhead
R-1 & 5 台同期演奏           & T4 で聴感確認．同期誤差・通信遅延を実測（TP-2, TP-3） \\
R-2 & 輪唱曲（主旋律 3 + リズム 1 以上） & 4 パート楽譜を用意し T4 で演奏 \\
R-3 & 可変テンポ追従          & 指揮 BPM の階段変化に対しテンポ追従を実測（TP-4） \\
R-4 (S) & 強弱制御            & 指揮振幅の変化に対し強弱分離を確認（TP-8） \\
R-Internal-1 & 通信の信頼性    & 5\% パケロス下で演奏継続を確認（TP-5） \\
R-Internal-2 & CPU リソース    & 入力フェーズ $\leq$ 2 ms を実測（TP-6） \\
R-Internal-3 & 起動時間        & 起動 5 s 以内を実測（TP-7） \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
BEAT 連発回数            & \texttt{beatRedundancy = 2}                & 同期誤差の評価で「2 で十分」を確認後に確定．T3 のパケロス実測値を見て 1--3 連発に調整 \\
