...
\begin{itemize}
  \item 全体進捗率：50\%（実装フェーズの後半．今週はコードの実装まで進めた段階で，
        実機テスト（書き込み・動作確認）はこれから着手する）
  \item 進捗状況（今週はいずれもコード実装まで，実機テストは次フェーズ）：
    \begin{itemize}
      \item test\_v2 の低遅延化・パケロス分散・WiFi 再 join 堅牢化：コード実装完了（PR \#21，実機テストはこれから）
      \item test\_v3 ゲームモードの firmware（共通／node\_01／node\_02）：コード実装完了（PR \#22，実機テストはこれから）
      \item test\_v3 の Processing 画面（メニュー／ゲーム／結果／アナライザ）：実装中
    \end{itemize}
  \item 今週の主要成果（コード実装）：
    \begin{itemize}
      \item test\_v2 を低遅延化（発音先読み 50→30ms・音声バッファ短縮）し，連送の時間分散と
            WiFi 復帰時の再 join による取りこぼし対策を実装した（効果の確認は実機テストで行う）
      \item test\_v3 ゲームモードを新規実装：IMU ナビ（左右の振り＝メニュー移動／縦振り＝決定），
            テンポ維持精度の採点（0〜100 点），メトロノームガイドの自動フェード
      \item 楽器ノードに UI 中継パケット（新 type=4）を追加し，PC 側 Processing がゲーム画面を描けるよう実装した
    \end{itemize}
  \item 重大課題（影響度：中）：
    \begin{itemize}
      \item {}[コードは全 5 ノードでビルドが通る（コンパイル確認まで）が，実機での動作はこれから検証する．
            特に IMU ナビの判定軸・符号・しきい値は実機の振り方に依存するため，実機書き込みでの調整が必要
            （設定値で差し替え可能にしてある）]
    \end{itemize}
\end{itemize}
...
作業項目（フェーズ）
 ...
 & \cellcolor{barPlan}\tiny 6/17 & \cellcolor{barPlan}\tiny 6/24 & \cellcolor{barPlan}\tiny 7/8 \\
\midrule
企画・テーマ決定        & \bD & \bD &     &     &     &     &     &     &     &     &     &     \\
110 基本設計            &     &     & \bD & \bD &     &     &     &     &     &     &     &     \\
120 詳細設計            &     &     &     &     & \bD &     &     &     &     &     &     &     \\
...
自由演奏（test\_v2）の実装が固まったため前倒しで着手した．
※ 200 実装の各バーは「コードの実装」までを指す．実機テスト（書き込み・動作確認）は
これからで，計画上は 300 テストフェーズで実施する．
...
\midrule
test\_v2 低遅延化・堅牢化 & 2026-05-27 & 2026-06-01 & 実装完了 & 100\% & 発音先読み 50→30ms・連送の時間分散・WiFi 再 join．コンパイル確認まで．PR \#21 \\
test\_v3 ゲームモード設計 & 2026-05-30 & 2026-06-01 & 完了 & 100\% & CTRL 予約 4B を mode／navCursor／targetBpm／score にフィールド化 \\
test\_v3 firmware 実装 & 2026-06-01 & 2026-06-01 & 実装完了 & 100\% & IMU ナビ・採点・UI 中継．全 5 ノード \texttt{pio run} 成功（コンパイルまで）．PR \#22 \\
test\_v3 Processing 画面 & 2026-06-02 & 2026-06-09 & 進行中 & 30\% & メニュー／ゲーム／結果／アナライザの画面遷移を実装中 \\
実機テスト・IMU ナビ調整 & 2026-06-上旬 & — & これから & — & 全ノードへ書き込み，動作確認．振りの軸・符号・しきい値を実機でチューニング \\
\bottomrule
\end{tabularx}
...
UDP マルチキャストのパケロス & マルチキャストは ACK・再送が無く，取りこぼしが音抜けになる & 中 & 中 & 連送 4 発を 2ms 間隔で時間分散し，WiFi 復帰時にマルチキャスト購読を貼り直す堅牢化を実装（先週からの継続）．効果は実機テストで確認する & 対応中 \\
...
\begin{itemize}
  \item 目標：
    \begin{itemize}
      \item {}[今週実装したコード（test\_v2 低遅延化・test\_v3 ゲームモード firmware）を全ノードへ
            書き込み，実機テストを行う．まずは自由演奏が従来どおり鳴るか，次にゲームモードが通るかを確認する]
  ...
  \item 達成基準：
    \begin{itemize}
      ...
      \item {}[実機でメニュー→ゲーム→結果の遷移が破綻なく動く]
    \end{itemize}
  \item 期限：
    \begin{itemize}
      \item {}[6 月中旬までに実機テストと Processing 画面・IMU 調整を終え，テストフェーズ（300）へ移行]
    \end{itemize}
\end{itemize}
...
\begin{itemize}
  \item {}[今週はコードの実装が中心で，動作はコンパイル確認までにとどまる．実機テスト（書き込み・動作確認）は次フェーズで行う方針]
