...
\begin{itemize}
  ...
  \item \textbf{PC}（Processing による音響合成~\cite{processing_sound}）$\times\,1$ 台
\end{itemize}
...
本稿は筆者が担当する**指揮者マイコン部**の設計方針に焦点を絞り，IMU によるジェスチャ取得（\ref{subsec:imu_gesture}）と WiFi/UDP ブロードキャストによる配信（\ref{subsec:wifi_udp}）という 2 つの技術選択を中心に述べる．
...
加速度と角速度を 100\,Hz 以上でサンプリングし，ノイズ除去後に加速度ノルムのピーク検出によって拍タイミングを抽出する．拍間のインターバルを移動平均してテンポを推定し，ピーク時の加速度振幅から強弱パラメータを導出する．IMU 信号から身体動作を識別する手法は uWave~\cite{imu_gesture_survey} をはじめとして広く研究されており，本提案でも加速度ノルムに基づく簡便なアルゴリズムから着手し，必要に応じて学習ベース手法への置き換えを検討する．

\subsection{WiFi/UDP ブロードキャストによる配信}
\label{subsec:wifi_udp}

指揮者マイコンから楽器マイコン群への拍情報配信には，WiFi 上の UDP ブロードキャスト~\cite{udp_realtime} を用いる．UNO R4 WiFi は ESP32-S3 を無線コプロセッサとして内蔵しているため，追加ハードウェアなしで同一サブネット上の全ノードに対し単一パケットで同時に送信できる．

UDP はコネクションレスで再送を行わないため遅延が小さく，1 対多の同時配信に適する．パケットロスが発生した場合も拍は周期的に送出されるため，次拍で自然に復帰でき，合奏の継続性を損ないにくい．合奏におけるアンサンブル精度の観点からは，片道遅延は数十ミリ秒以下に抑えることが望ましく~\cite{music_latency}，本提案ではこれを要件として次節で数値化する．
...
これらは合奏の許容同期ズレ~\cite{music_latency} を内訳として配分した目安であり，実機での実現可能性は次週以降の個人タスクにおいて計測・評価する．
