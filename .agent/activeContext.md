...
- **結合レポート 最終チェック → 即修正可5件を修正済み**（2026-05-21）。15件の指摘のうち
  高①（ref:douki→河瀬2014差し替え）・高②（NOTE表instrumentId）・中⑤⑥（強弱の表現）・
  低⑩（拍検出表現）を修正しコミット（b678370・53fbf7c）。ビルド49ページ・Overfull0維持。
  残10件は事実確認/方針判断/図修正で未着手。指摘全文は `_作業計画.md` §6-6（Phase 6）。

## 次の一手

- 残10件の振り分け（`_作業計画.md` §6-6「進め方・進捗」）:
  - 要事実確認: 高③ref:onkai 5.52cent の出所 → 齋藤へ。
  - 方針が決まれば即修正可: 中④state再採番・中⑧楽器台数注記・低⑬図の表記ゆれ。
  - 要チーム判断: 中⑦課題曲・中⑨MOE音階指標（高③と連動）・低⑪状態遷移図の節名。
  - 据え置き提案: 低⑫楽譜idx式・低⑭アロー図・低⑮CTRL予約注記。
- 本体修正は .tex 編集 → `latexmk -lualatex` → PDF確認 → コミット。
  図修正は `fig/<name>.drawio` 編集 → draw.io CLI で PNG 再書き出し → 両方コミット。
