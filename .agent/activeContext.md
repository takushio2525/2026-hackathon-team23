...
1. 最終レポート・振り返り（7/15）へ MOP_REPORT_20260711.md の内容を転記
   ...
   発表資料には `results/graphs/mop5_fire_delay_by_node_slide.png`（スライド用・
   データのみのシンプル版、口頭解説前提）を使う。体裁はユーザー指定で
   「通信遅延（ノード別）」タイトル・平均値の青棒・最大値の赤マーク（p95/中央値なし）。
   単体で読める詳細版は `mop5_fire_delay_by_node.png`（説明・判定・注記入り、p95 基準のまま）。
   好評なら他 MOP も同体裁でグラフ化する。
