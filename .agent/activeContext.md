...
- **塩澤の個人作業ログ第1回をパケロス重点へ全面リライト**（2026-05-26、
  `work/shiozawa/work-0525/作業ログ0525/作業ログ0525.tex`）。スコープを
  「test_v1 完了まで」→「test_v1 完了 + test_v2 のパケロス調査」に拡張し、
  重大課題を WiFi 到達ばらつき（解決済）→ UDP マルチキャストのパケロス
  （未解決・対応中）に全面置換。作業ログ表に「3 声輪唱 検証（test_v2）」と
  「パケロス計測・対策」の 2 行を追加、課題管理表でパケロスを最上段に。AI 利用・
  検証方法・ファクトチェック・次回計画もすべてシーケンス番号付与＋同一パケット
  連送、NOTE のユニキャスト分離検討の話に書き直し。Docker latexmk で
  6 ページ・Overfull 0 でビルド成功（Underfull は表組み内の和文行末の微小なもののみ）。
- 直前まで: 個人作業ログ第1回（test_v1 まで）を初版作成（d10b131・2026-05-26）。
