...
1. **ビーコン 50TU 設定ブロックの撤去**（`OrcNetModule.cpp:29-51` の 28 行 revert 相当）→ 指揮者だけ
   再書き込み → 同条件（lookahead 220 + min フィルタ、`--lookahead 220`）で再計測。
   ストールが消えれば発火 p95 ≈9ms（MOP5 PASS 圏）・MOP4 p95 ≈11〜20ms の見込み。
2. 残るなら MOP8 要領のループハートビート計測でストール粒度・窓位置を確定 → 発火判定の
