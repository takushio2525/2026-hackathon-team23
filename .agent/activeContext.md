...
- **test_v3 ゲームモード 設計フェーズ（Phase 1 残り）を完了**（2026-06-01、`shiozawa-test_v3-game`
  ブランチ）。`.agent/test_v3-game-design.md` を新規作成。(a) CTRL 予約4B 割当（mode/navCursor/
  targetBpm/score・画面は state を Menu/Result 拡張して導出）、(b) node_01 IMU ナビ（左右=カーソル/
  縦=決定、新 Menu/Result state で拍検出と排他）、(c) node_02 UI 中継（新 type=4 PKT_UI、変化時＋最大5Hz）、
  (d) Processing 役割自動判定（手動選択廃止・partId/UIフレームで判定）を設計。**重要な逸脱**: 計画書 Phase 4A は
  予約4B=mode/targetBpmQ8(2B)/score だったがカーソルの空きが無く、targetBpmQ8→targetBpm(1B) に簡素化。
  master 確認事項として明記済み。実装は Phase 2 以降で master 指示後（鉄則: コンパイルまで、実機はユーザー）。
...
- **test_v3**: master の指示を待って Phase 2 着手（設計メモ §8 の実装分解順）。1=共通 OrcProtocol
  （PKT_UI/CtrlPayload）、2=node_01（Menu/Result state・GameData・ナビ・採点・ガイド）、3=node_02
  （CtrlData 拡張・UiRelayModule）、4=Processing（役割自動判定・4 画面＋アナライザ・メトロノームフェード）。
  着手前に `.agent/test_v3-game-design.md` を Read。master の確認待ち事項は同メモ §9。
