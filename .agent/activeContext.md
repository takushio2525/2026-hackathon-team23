# 現在の作業文脈

> このファイルは **毎ターン上書き**される動的文脈ファイル。
> 履歴は `progress.md` に追記する形で残す。

## 現在の対象

- **結合レポート 提出前の最終チェック実施**（2026-05-21）。本体49ページ＋全10図＋
  ファーム実コード＋議事録＋参考文献6件を突き合わせ，**15件の指摘**（高3・中6・低6）を抽出。
  LaTeX体裁（Overfull/Underfull 0・未定義参照0）とコード整合は良好だが，**指摘は未修正**で
  ユーザーの修正方針待ち。指摘の全文は `_作業計画.md` §6-6（Phase 6）に起票済み。

## 次の一手

- ユーザーが修正方針を決め次第，本体 `23_計画書・設計書.tex` を修正 →
  `latexmk -lualatex` → PDF確認 → コミット。
- 即修正可（事実確認不要）: 高①ref:douki引用是正・高②NOTE表へinstrumentId追加・
  中⑤⑥強弱の表現統一・低⑩⑫⑬⑭⑮。
- 要事実確認: 高③ref:onkaiの5.52cent出所（齋藤へ）。
- 要方針判断: 中④state再採番・⑦課題曲・⑧楽器台数注記・⑨MOE音階指標・低⑪状態遷移図。
- 図修正時は `fig/<name>.drawio` 編集 → draw.io CLI で PNG 再書き出し → 両方コミット。

## 現フェーズで Read すべき設計書

- 経緯・全方針: `report/計画書_中間発表/_作業計画.md`（Phase 0〜5 完了済み）。
- 本体修正時: `report/計画書_中間発表/23_計画書・設計書.tex`。

## draw.io ワークフロー（再修正時の参照）

- 書き出し: `/Applications/draw.io.app/Contents/MacOS/draw.io --export --format png
  --scale 2 --crop --border 14 --output 出力.png 入力.drawio`
- `.drawio` と `.png` を両方コミットする。`.png` を手で描き換えない。

## ガント図の整合基準（再修正時の参照）

- バー週数は **アロー図 `arrow.drawio` が正**: 110/120=各1.5週，210=0.5，220=1.0，
  230=0.5，240=1.0，250/260=各1.5，310/320=1.0，330=1.0，510=1.0週。図上 1週=66px。
- 絶対日付は授業スケジュール: 計画発表5/20・実装5/27〜・評価会6/24・発表会7/1。
- フェーズ工数は WBS 表: 設計3週・製造2週・テスト2週。
- クリティカルパスは逐次（階段配置）。240/250/260 は製造期間に並行・フロートあり。

## ユーザーの好み

- 大規模作業は計画を作ってから着手。設計章は大まかに（内容が伝われば可）。
- 図は draw.io に一本化（TikZ 廃止済み）。表が読みやすければ図を表化してよい。
- 短い指示は行間を読み，前提のズレを感じたら着手前に指摘する。

## 既知の論点

- 本物画像（`計画書結合/` 配下）は楽器4台・実装5/13開始の旧計画。`ganto.png` には
  「260ゲーム要素」が存在する（旧記述「本物画像は全点ゲームなし」は不正確）。本物画像は
  本体 .tex へ取り込まない（対比・日程感の参考のみ）。
- 状態遷移図は課題項目だが，ユーザー決定でシーケンス図を主図とした。節名は「状態遷移図」の
  まま，状態定義表3つを存置してフォロー済み。
- `.agent/api.md` の CTRL 予約 4B は未修正（現行ファームの事実）。ゲーム拡張は報告書側の将来設計。
- `.gitignore` に例外 `!report/計画書_中間発表/23_計画書・設計書.pdf` あり。本体 PDF はコミット対象。
- 図インベントリは全10点 draw.io（`_作業計画.md` §8）。
