...
Embedded-Module-Architecture（以下 **EMA**）を全面採用し、5 台のマイコン
（指揮者 1 ＋ 楽器 4）と PC アプリ（Processing）を UDP マルチキャストで結合する。
...
- **PC**: 各楽器からの NOTE を `sound_lab/data/*.json` の音色定義で加算合成 → スピーカ
...
- `instrumentId`: 0（金管）/ 1（木管）/ 2（弦）など。PC 側 `data/*.json` の音色キーに対応
...
- 役割: 各楽器ノードからの NOTE を USB シリアルで受信 → `sound_lab/data/<instrumentId>.json` の倍音定義で加算合成 → スピーカ
- ADSR エンベロープで自然な発音
...
- BEAT は同一内容を 2 連送（`beatRedundancy = 2`）
