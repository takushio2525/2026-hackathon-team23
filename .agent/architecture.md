...
Embedded-Module-Architecture（以下 **EMA**）を全面採用し、4〜5 台のマイコン
（指揮者 1 ＋ 楽器 3〜4）と PC アプリ（Processing）を UDP マルチキャストで結合する。

- **test_v2（現行・推奨）**: 指揮者 1 + 楽器 3（`node_02〜04`）の 4 台構成。きらきら星 3 声輪唱で稼働中。
- **production（本番想定）**: 指揮者 1 + 楽器 4（`node_02〜05`）の 5 台構成。雛形のみ。
