...
- 2026-07-15: **最終報告書の図清書・回路図差替・DevKitC整合**（`086e34e`）。drawio 図 8 枚を白黒・標準記法へ
  清書，回路図を KiCad PNG へ差替（旧 hardware 削除），DevKitC-1 主構成へ全箇所統一，全図表 [H] 配置化。
  Docker latexmk Error 0・Overfull 0・62ページ。

- 2026-07-15: **個人最終報告書をサンプル報告書準拠に全面改稿**（`c39497b`・`ff7c235`、61ページ）。
  図22枚（実測グラフ12・FC4・原理図ほか）・表11・付録Bを新設。MOP5は当初定義で未達報告に改め
  原因・対策は考察へ、エンタメアンケート43件の節を追加、PlatformIO採用理由を明記。全12文献の
  実在・引用内容をWebで確認、全数値をresults/正本と再集計一致確認。latexmkエラー0・Overfull 0。

- 2026-07-12: **最終報告書のルーブリック指摘 2 件を修正**（`1f6336b`）。§4.4 に文献 [6] の \cite{ref:onkai}
  と由来 1 文を追加、§2.1・図 1 の「金管4声部」を「4声部（声部1〜4）」へ修正（drawio 再書き出し）。
  チェックリスト 2-7 の誤記録も訂正。Docker latexmk エラー 0・未定義参照 0・Overfull 0・43 ページ、
  全 10 bibitem の引用を grep 確認。

- 2026-07-12: **最終報告書のルーブリック評価を作成**（`work/shiozawa/最終報告書/ルーブリック評価.md`）。
  採点者視点で全 11 項目を判定し総合 90/100（A×9・B×1、倍率 ×1 見込み）。スポットチェック 14 件で数値の
  虚偽 0 件（MOP 正本・実コード全一致）。唯一の明確な失点は文献 [6] の本文未引用（チェックリスト確認.md
  2-7 の ✅ は誤記録）。修正提案 8 件を優先度順に記載、報告書本体は無変更。

- 2026-07-12: **塩澤の個人最終報告書を新規作成**（`work/shiozawa/最終報告書/`、43 ページ）。教員配布
  テンプレ (jlreq+uplatex) 準拠、MOP 全 9 項目の実測・計画との相違点と理由・生成 AI 検証過程・付録に担当分
  中核ソース 4 本を収録。Docker latexmk EXIT=0・未定義参照 0・Overfull 0、checklist.pdf 全項目確認済み
  （`チェックリスト確認.md`）。pckgs.sty ローカルコピーのみ原ノ味構成へ差し替え（テンプレ原本不変）。

- 2026-07-12: **MOP4 スライド用グラフの数値ラベル配置を整理して視認性を改善**（`71a02fb`）。平均・中央値は
  「名前+数値」を線の右端外へ、最大は矢印付きで棒の真上へ移して被りを解消。中央値は右外◇→濃色点線の
  水平線、凡例は 2 項目に整理。数値（平均 10.8/中央値 7/最大 65/173 拍）と集計は不変、縮小表示でも判読確認。

- 2026-07-12: **MOP2 スライド用グラフを棒 1 本構成に簡素化**（`7f3811a`）。中央値◇・最大赤マークを削除し
  「音階誤差」青棒＋数値ラベル＋合格範囲緑帯のみへ（誤差ばらつきが小さく 3 統計量を並べる意味が薄いため）。
  summary 出力は不変、数値 0.86/1.91/0.10/0.08 cent は summary と一致確認。

- 2026-07-12: **MOP2（音階誤差）の検証プログラムを新規作成し PASS**（`mop2_pitch_error.py`）。Processing の
  加算合成を Python へ忠実移植して合成 → 基音推定 → 平均律と比較（実機・録音不要）。全 24 音の平均 |誤差|
  0.737 cent・最大 1.907 cent < 基準 3.6。誤差の正体は `harmonics[n=1].ratio ≠ 1.0`（ホルン −1.91・
  トランペット −0.87 cent）で、閉形式の手計算・独立 2 推定手法と一致。スライド用グラフ 1 枚 + evaluation.md。

- 2026-07-12: **成果発表向けdocsをproduction最終仕様へ同期**。220ms発音予約・2秒窓の最小遅延時計同期・
  最終MOP4/5を反映し、発表の要点／想定問答（根拠・限界・答え方）を新設。Astro build 86ページ SUCCESS。

- 2026-07-11: **スライド用 MOP グラフ 2 枚を新しい見せ方に再構成**（`ff81129` MOP5 /
  `1dd29c6` MOP4）。MOP5 は「ジッタ吸収なし（受信即発音なら平均 101.2ms）vs あり（実測 12.1ms）」
  の 2 グループ比較、MOP4 は最終構成 173 拍の時系列で各拍の最速 vs 最遅の発音時刻差
  （平均 10.8/中央値 7/最大 65ms）。全数値 CSV 独立再計算・レポート整合、詳細版 PNG は無変更。

- 2026-07-11: **MOP4 同期誤差グラフのスライド用シンプル版を追加**（`0fb4519`）。MOP5 スライド版と
  同体裁（青棒=平均・白抜き◇=中央値・赤マーク=最大・緑帯 ≤20ms）、x 軸は 3 回実測の計測回別
  （平均 9.8/10.1/10.8・p50 8/7/7・最大 32/79/65ms、CSV 独立再計算とレポート表の全一致を確認）。
  MOP5 の 3 PNG は MD5 一致で無変更。

- 2026-07-11: **MOP5 スライド用グラフにノード別中央値マークを追加**（`f8612b4`）。白抜き◇＋濃縁
  （zorder 5）で青棒上でも視認可、数値 5/8/6/6/5ms は棒の右外側配置、凡例 4 項目化（ncol=4）。
  CSV 独立再計算と一致、詳細版・mop5_comm_delay はバイト無変更。

- 2026-07-11: **MOP5 スライド用グラフをユーザー指定体裁へ変更**（`bffbd99`）。タイトル
  「通信遅延（ノード別）」・平均値の青棒・最大値の赤マークに置換（p95/中央値◇削除）。
  平均 10.8/15.2/12.6/11.3/10.3ms・最大 58/66/67/58/58ms は CSV 独立集計と一致、詳細版はバイト無変更。
...
- 2026-07-11: **発声タイミング遅延（MOP5 発火 lateMs）の第三者向けグラフを作成**
  ...
  数値は `mop5/20260711_154006_summary.txt` と全一致を確認。他 MOP 横展開は好評なら実施。

- 2026-07-11: **最終 MOP 検証レポートを作成**（`results/MOP_REPORT_20260711.md`）。MOP4/5 を 7/10-11 の
  3 回実測（延べ 560 拍 × 5 ノード）で更新: MOP4 は実質達成・受け入れ（p50 7ms、尾はストール由来 9.2%）、
  MOP5 は指標のスコープミスと総括し「予約成立」へ再定義（受信遅刻率 45.4→1.9〜3.1%）。ストール深掘りは
  ユーザー判断で中止、M45S 計測は将来手段として存置。数値は全ログから再集計・summary 一致確認。
...
- 2026-07-10: **MOP5 対処案 1/2/4 を production ファームに実装**（`35f70f0` lookahead 45→220ms /
  ...
  ビルド SUCCESS。実機書き込み・再計測は master 側（--lookahead 220 指定が必要）。
...
- 2026-06-24: **本番プログラム構築（`feature/production-program` ブランチ、4コミット）**。
  齋藤版 week9 楽譜のベロシティ反映（`b8295dd`）、ドラム音色 JSON 更新（`180967f`）、
  ドラムノード node_06 新設（`a8b18d3`）、マスターリセット検知修正（`a4a6dd1`）。
  全7ノード pio run SUCCESS。

- 2026-06-24: **本番プログラム レビュー＆リファクタ（3コミット）**。
  Processing ドラム音色の noteNumber 振り分け追加（`2beca0c`）、
