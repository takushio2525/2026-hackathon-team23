...
| 17 | 1 B | `navCursor` | **test_v3**: メニューカーソル位置 0..N（Menu/Result で有効）。**state=Conducting の間はジャイロ角速度 X（int8, gyro[0]/8）が透過する**（PC の指揮棒 2D プロット用） |
| 18 | 1 B | `targetBpm` | **test_v3**: ゲーム目標テンポ（生 BPM 40–240、0=未設定/自由演奏では無視）。※計画書の `targetBpmQ8`(2B) はカーソル領域確保のため 1B 生 BPM に簡素化（案A） |
| 19 | 1 B | `score` | **test_v3**: ゲーム得点 0–100（0xFF=採点中/未確定。Menu/Result で有効）。**state=Conducting の間はジャイロ角速度 Y（int8, gyro[1]/8）が透過する** |
...
| オフセット | サイズ | フィールド | 内容 |
|---|---|---|---|
| 12 | 1 B | `partId` | ノード ID。test_v2/v3 は `0x02`〜`0x04`（楽器 3 台）、production 想定は `0x02`〜`0x06`（楽器 5 台＝金管 4 + ドラム）。輪唱のどの声部か |
...
| 14 | 1 B | `navCursor` | メニューカーソル位置（Conducting 中は CTRL 由来のジャイロ X が透過） |
| 15 | 1 B | `targetBpm` | ゲーム目標テンポ（生 BPM） |
| 16 | 1 B | `score` | 0–100 / 0xFF=未確定（Conducting 中はジャイロ Y が透過） |
| 17 | 1 B | `partId` | 中継元ノード ID（PC の役割判定用。`0x02`=メイン操作 UI） |
| 18 | 2 B | `bpmQ8` | 実振り BPM ×8（演奏画面のテンポ表示用） |

UI は **node_02 のみ**が USB シリアルで PC へ中継する（node_03/04 はアナライザで NOTE のみ）。
**UDP マルチキャストには一切流さない**ため、同期経路（CTRL/BEAT/NOTE）に影響しない。送出は
「内容が変化したとき（最小間隔 33ms）＋1s heartbeat の保険送出」。Menu/Result 中は操作ペースの
低頻度だが、Conducting 中はジャイロが毎 CTRL 変化するため実質 30Hz の連続送出になる
（20B×30Hz=600B/s ≪ 115200bps で NOTE バーストを阻害しない）。Processing は
...
`firmware/test_v3/node_0{2,3,4}/include/score_data.h` で定義され、
