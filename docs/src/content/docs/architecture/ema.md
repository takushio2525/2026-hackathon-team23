...
| node_02〜04（楽器） | `OrcReceiverModule`、`NoteSenderModule`（楽譜進行は `src/applyPattern.cpp` で `OrcReceiverData` を消費） |
...
- 新しい `IModule` を足す手順 / 落とし穴 → [モジュール拡張ガイド](/deep-dive/module-extension/)
