...
`bpmQ8` (×8 整数表現、分解能 0.125 BPM) を float に戻す。例：`bpmQ8 = 964` → `bpm = 120.5`。
（フィールド名に `Q8` を含むが、一般的な Q8 固定小数 = ×256 ではない）
...
これにより：
- 連送（`beatRedundancy = 4`、2026-05-25 暫定値）の 2 個目以降を破棄
- ネットワーク経由で偶発的に複数回届いたパケットを破棄
...
- もし重複でこれを更新しないと、連送の 2 個目以降を捨てた後に「もう BEAT が来ていない」と
  誤判定する
