...
**Arduino マイコン + PC 1 台で動く、指揮者の腕の振りに合わせて演奏する電子オーケストラ**
（現行 test_v2 は合計 4 台 = 指揮 1 ＋ 楽器 3、production 想定は合計 6 台 = 指揮 1 ＋ 楽器 5）。

- 指揮者役のマイコン（1 台）が IMU で腕の振りを読む
- 楽器役のマイコン（**現行 test_v2 は 3 台、production 想定は 5 台 = 金管 4 ＋ ドラム 1**）が、拍に合わせて自分の楽譜を進める
...
> production 想定では `node_05`（金管 4）と `node_06`（ドラム）を追加して **楽器 5 台構成**
> にする予定（[ADR-0004](/decisions/0004-ensemble-structure/) 改訂版・本実装未着手）。
...
### 楽器ノード — Arduino UNO R4 WiFi（現行 test_v2 は ×3、production 想定は ×5）
...
> 楽器ノード（現行 3 台 / 本番想定 5 台）はすべて **同じ楽譜データ** を持っている。
