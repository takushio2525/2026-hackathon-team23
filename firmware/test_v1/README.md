# firmware/test_v1 — Arduino オーケストラ仕様書準拠の最初のテスト実装

> これは最初の同期検証版（旧 `firmware/test`）。きらきら星の輪唱 + 楽器番号付き NOTE の続編は
> [`../test_v2/`](../test_v2/)（PC 側は `pc_app/test_v2/`）。新しく動かすならそちらを使う。
...
```bash
# 1. 指揮者ノードを書き込む
cd firmware/test_v1/node_01
pio run -t upload

# 2. 各楽器ノードを書き込む (それぞれ別の Mac から)
pio run -d firmware/test_v1/node_02 -t upload   # 金管 1
pio run -d firmware/test_v1/node_03 -t upload   # 金管 2
pio run -d firmware/test_v1/node_04 -t upload   # 木管 1

# 3. 各 Mac で Processing を起動し
#    pc_app/test_v1/orchestra_player/orchestra_player.pde を Run
