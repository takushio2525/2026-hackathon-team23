# node_04 — 楽器 3 (木管 1) ノード (テスト版)

Arduino UNO R4 WiFi で「木管 1」パートを担当するテスト用実装。
...
- 楽譜: ソラシドシラソ (G4 A4 B4 C5 B4 A4 G4) — node_02 (C4 ベース) の 5 度上、
  node_02 + node_03 と同時演奏で C major triad のハモリになる
...
- 周期 (200 ms): `[N4 t=… st=Playing wifi=1 sync=ok(off=… n=…) ctrl=(bpm=… v=… s=…)
  recv=(no=… ago=…) pend=… score=(idx=… snd=…)]`
- イベント:
  - `[N4 EVT STATE]` Idle/WaitStart/Playing の遷移
  - `[N4 EVT WIFI]` STA リンクの up/down
  - `[N4 EVT SYNC_CONVERGED]` 時計同期収束時の offset と sample 数
  - `[N4 EVT CTRL]` / `[N4 EVT BEAT]` パケット受信ごとに seq・bpm・playAt 等
  - `[N4 NOTE_ON ]` / `[N4 NOTE_OFF]` 発音タイミングと note/vel/dur

`SERIAL_DEBUG=0` のときは従来どおり 20 B バイナリ NotePacket を Serial に書き、
`pc_app/test_v1/orchestra_player/` (Processing) が読み取って音を鳴らす。
...
├── platformio.ini
├── include/
│   ├── ProjectConfig.h     # 設定一元化 (partId=0x04, startBeatNo=0)
...
他の楽器ノード (node_02 / 03 / 05) はこのコードをコピーし、
