# node_02 — 輪唱 声部 1（test_v2）

Arduino UNO R4 WiFi で「きらきら星」輪唱の **声部 1** を担当する。曲頭から入る。
...
node_03 は `headRestBeats=8 / instrumentId=1`、node_04 は `headRestBeats=16 / instrumentId=2`。
...
- **楽譜の進み方**: 「指揮者の拍番号 `firedBeatNo`」から自分の楽譜インデックスを計算する
  （`effective = firedBeatNo - 1 - headRestBeats`、`effective < 0` の間は休符＝無音、
  `effective >= 0` なら `kScore[effective % 48]` を発火）。拍番号で引くので、PC 側 Processing を
...
```bash
pio run -d firmware/test_v2/node_02                  # ビルド
pio run -d firmware/test_v2/node_02 -t upload        # 書き込み
pio device monitor -d firmware/test_v2/node_02       # 注: Processing と同時に開けない (ポート競合)
```
...
│   └── score_data.cpp      # 楽譜本体 — きらきら星 全曲 (kScore[], 48 拍)。3 台とも同一
└── lib/
    ├── OrcReceiverModule/  # CTRL/BEAT を整形して Sync / Receiver / Ctrl に書く
    └── NoteSenderModule/   # NOTE (楽器番号/長さ/高さ…) を USB Serial へ送出
```

node_03 / node_04 はこのコードのコピー。差し替えるのは `include/ProjectConfig.h` だけ
