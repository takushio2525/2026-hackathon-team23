...
enum class ConductorState : uint8_t {
    ...
    Conducting  = 2,
    Fallback    = 3,
    Menu        = 4,   // production: IMU ナビでモード選択中 (拍検出は止める)
    Result      = 5,   // production: ゲーム結果表示中 (得点を凍結・縦振りで Menu へ)
};
...
// production ゲームモードの司令塔状態。CTRL 予約バイト (mode/navCursor/targetBpm/score) の源。
// 詳細設計は .agent/production-game-design.md。
struct GameData {
...
struct SystemData {
    ...
    GameData            game;       // production: ゲームモード状態
};
