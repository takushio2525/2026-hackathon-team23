...
- テンポ推定 (test_v2): 最初の 1 拍 (= 1 音目) はまだ拍間隔が取れないので **初期値 100 BPM**
  で CTRL を流す。2 拍目で「1→2 拍目の間隔」をそのまま簡易テンポとして確定し、3 拍目以降は
  拍ごとに瞬時 BPM を EMA (α=0.30) で随時補正、40–240 BPM にクランプ
...
`firmware/test_v2/common/lib/` を `lib_extra_dirs` 経由で参照する。
