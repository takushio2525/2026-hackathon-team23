...
void dumpEdges(const SystemData& d) {
    ...
    if (d.orcNet.hasNewBeat && d.receiver.lastBeatNo != gPrevLastBeatNo) {
        ...
        DBG_PRINTF("[N6 EVT BEAT] no=%u playAt=%lu ahead=%ld seq=%lu\n",
...
void dumpPeriodic(const SystemData& d) {
    ...
    DBG_PRINTF(
        "[N6 t=%lu st=%s wifi=%d sync=%s(off=%ld n=%u) ctrl=(bpm=%5.1f v=%u s=%u) "
...
void setup() {
    ...
    DBG_PRINTLN("");
#if MOP_TEST == 7
    mop_test::mprintf("M7,6,BOOT,%lu\n", (unsigned long)millis());
#endif
    DBG_PRINTF("=== node_06 (drum partId=0x%02X instr=%u headRest=%u) boot ===\n",
    ...
    DBG_PRINTLN("[N6 INIT] done");
#if MOP_TEST == 7
    mop_test::mprintf("M7,6,INIT,%lu\n", (unsigned long)millis());
#endif
}
