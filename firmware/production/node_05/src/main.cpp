...
void setup() {
    ...
    DBG_PRINTLN("");
#if MOP_TEST == 7
    mop_test::mprintf("M7,5,BOOT,%lu\n", (unsigned long)millis());
#endif
    DBG_PRINTF("=== node_05 (round voice partId=0x%02X instr=%u headRest=%u) boot ===\n",
...
void loop() {
    ...
    if (!sMop7Wifi && gData.orcNet.wifiConnected) {
        mop_test::mprintf("M7,5,WIFI,%lu\n", (unsigned long)millis());
        sMop7Wifi = true;
    }
    if (!sMop7Sync && gData.sync.converged) {
        mop_test::mprintf("M7,5,SYNC,%lu\n", (unsigned long)millis());
        sMop7Sync = true;
    }
    if (!sMop7Ready && gData.receiver.hasFirstBeat) {
        mop_test::mprintf("M7,5,READY,%lu\n", (unsigned long)millis());
        sMop7Ready = true;
    }
#endif
