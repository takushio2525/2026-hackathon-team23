...
```bash
pio run -d firmware/production/node_05                  # ビルド
pio run -d firmware/production/node_05 -t upload        # 書き込み
pio device monitor -d firmware/production/node_05       # 注: Processing と同時に開けない (ポート競合)
```
