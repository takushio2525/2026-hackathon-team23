...
- 役割: 指揮者ノード node_01 の SoftAP に STA で接続し、CTRL/BEAT を受信、
  ...
  Mac (Processing) に送出
- partId: `0x02` (金管 1)
- startBeatNo: `0` (このパートは曲頭から / `beatAt` は startBeatNo+1 始まり)
- 状態遷移: Idle → WaitStart (時計同期収束 + 入り拍到来) → Playing
  (Playing からは戻らず、BEAT が来ない間は単に発音を止めて待つ)
- 進行則: **「BEAT を 1 個受信したら `effective` 拍 (== 比較) のイベントだけを発火する」**
  純粋イベント駆動。BPM 起点の自走 (旧 SelfRun) は持たない。同一 `beatAt` に
  複数行を並べれば和音 / 同時発音、`subNote` を立てれば 8 分音符の細分発火を表現できる。
