...
- 状態遷移: Idle → WaitStart (時計同期収束 + 入り拍到来) → Playing
  (Playing からは戻らず、BEAT が来ない間は単に発音を止めて待つ)
- 進行則: **「BEAT を 1 個受信したら score を 1 拍だけ進める」純粋イベント駆動**。
  BPM は CTRL に含めて送られるが、楽器側は durationMs 計算や将来の 8 分音符
  細分化など補助用途にしか使わず、BPM 起点の自走 (旧 SelfRun) は持たない
...
**この間 Serial は人間可読テキスト専用となり、Processing 連携用の 20 B バイナリ
NotePacket は流れない**。Processing と接続したいときは `-DSERIAL_DEBUG=0` に
変更してビルドし直すこと。

`SERIAL_DEBUG=1` で `pio device monitor` を開くと以下が流れる:

- 起動時: 各モジュール `init()` の OK/NG
- 周期 (200 ms): `[N2 t=… st=Playing wifi=1 sync=ok(off=… n=…) ctrl=(bpm=… v=… s=…)
  recv=(no=… ago=…) pend=… score=(idx=… snd=…)]`
- イベント:
  - `[N2 EVT STATE]` Idle/WaitStart/Playing の遷移
  - `[N2 EVT WIFI]` STA リンクの up/down
  - `[N2 EVT SYNC_CONVERGED]` 時計同期収束時の offset と sample 数
  - `[N2 EVT CTRL]` / `[N2 EVT BEAT]` パケット受信ごとに seq・bpm・playAt 等
  - `[N2 NOTE_ON ]` / `[N2 NOTE_OFF]` 発音タイミングと note/vel/dur
